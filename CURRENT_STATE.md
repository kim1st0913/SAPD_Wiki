...
- 2026-07-23 本地 MCP Web 优先 v0.2 批次 B `B0–B5` 已在独立 `codex/local-mcp-web-integration` 分支完成并通过 B5：5173 已接入真实 Sidecar、loopback HTTPS、本地 OAuth、授权确认、五项 Tool、真实控制状态和自有客户端端到端，自动化 `89/89`、宽/窄屏、键盘刷新、返回全局导航和 console 门禁通过；实现提交为 `aa51f78`。Web 验收使用 `memory://isolated-web-dev` 临时内存状态，未打开或创建用户库文件，正式/用户数据、系统信任、真实 Codex 配置、App、packaging、D0 和 push 均为 0。5173 保持隔离预览，MCP 当前停止在 `29875`；当前电脑/当前 Codex 真实验证仍需 C1 单独授权。现行计划为 `docs/06-implementation/local-mcp-web-first-development-and-client-validation-plan-v0.2.md`，PASS 报告为 `docs/06-implementation/local-mcp-b5-web-real-loop-pass-report-2026-07-23.md`，跟踪为 `OI-199`。
- 2026-07-23 本地 MCP 产品开发计划 v0.1 的批次 A 已在独立 `codex/local-mcp-product-development` 分支完成并通过原 D6 synthetic 门禁：正式只读知识服务、Sidecar、控制 API、Web synthetic 启停、macOS/Windows Supervisor 与桥接接口均已实现。T0–T2 `53/53`、D1 `24/24`、D2 `19/19`、D3/Web `24/24`、集成 `3/3`、签名 `7/7`、Electron `27/27`、Swift `17/17` 通过，macOS 完整构建通过；浏览器宽/窄屏、synthetic 启停、桌面能力门禁和零 console error 验收通过。原 dirty checkout 保持只读；真实/用户数据、真实客户端配置、系统信任、真实 OAuth、D0、packaging 和 push 均为 0。本条是历史证据，其后续执行顺序已由 v0.2/v0.7 覆盖。
- 2026-07-23 本地 MCP v0.5 的 G0 与 `T0 → T1 → T2` 已在独立 `codex/local-mcp-m0t` worktree 完成并停止在授权边界：G0、T0、T1、T2 阶段提交分别为 `d77a9f82 / 80df546f / c4493067 / e0b03f45`；四份合同、6 个 Schema、30 条 synthetic fixture 与 macOS 本机只读 Runtime / loopback HTTPS / OAuth / 五 Tool harness 通过，合并回归 `53/53`。没有 push、真实/用户数据访问、系统信任或真实客户端配置、App / packaging 修改；Windows 与 T3 真实 Codex 客户端仍 pending，D0-Pilot、M1 未授权。T0–T2 通过不等于 M0-T 总体 PASS，跟踪为 `OI-199`。
