...
- 2026-07-23 用户已确认本地 MCP 产品开发计划 v0.1 的批次 A，执行顺序调整为“完整开发 → 用户本机当前客户端验证 → 多版本/双平台验证 → 内容与发布”。批次 A 在独立 `codex/local-mcp-product-development` 分支执行 `P0 → D1 → D2 → D3 → D4 → D5 → D6`，允许正式只读服务、Sidecar、控制 API、Web“系统设置 → AI 集成”、macOS/Windows 桥接、synthetic 测试和本地阶段提交；不允许 push、真实客户端配置、系统信任、真实 OAuth、D0、真实/用户数据或 packaging。v0.6 补充决策只调整开发门禁顺序，v0.5 的安全、数据与发布边界继续有效；跟踪为 `OI-199`。
