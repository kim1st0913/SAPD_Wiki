...
- 2026-07-07 最新 macOS DMG 已基于当前工作区重新覆盖打包，正式版本保持 `0.1.6`，构建时间戳和文件名继续复用 `20260707-085841Z`，未升级版本号。授权版为 `apps/macos/SAPDWiki/dist/license/SAPD-Wiki-0.1.6-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `f414986d30f4ab52dcadb3247ed1ebb582277f3c3b59e67ffa5d34ab4f016792`；无授权版为 `apps/macos/SAPDWiki/dist/no-license/SAPD-Wiki-0.1.6-no-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `3b972ed53de4bf1a17d43ff5ab4efabb3d285cfb3ef5b7ebd20944e2eb0fe34c`；两包大小均为 `164M`。`package_dmg.sh` 继续默认一次生成 `license` / `no-license` 两个 DMG，版本号和时间戳一致，文件名和存储路径区分变体。授权版启动先弹授权窗口，正确授权码 `Passc0de` 后显示 `已激活` 且不受限制；不知道授权码可跳过进入 30 天试用；到期后不能跳过。无授权版不弹授权窗口，前端授权状态显示 `无限制版`。`Info.plist` 已统一为 `CFBundleShortVersionString=0.1.6`、`SAPDWikiDisplayVersion=0.1.6`，并通过 `SAPDWikiLicenseMode=license/no-license` 区分变体。该 DMG 为 ad-hoc signed 非公证包，外部分发仍可能触发 macOS `隐私与安全性` 手动允许打开。用户已否决正式 Developer ID 签名公证方案和安装 / 自动更新机制方案，按内测交付现状接受并归档 `OI-190`。
