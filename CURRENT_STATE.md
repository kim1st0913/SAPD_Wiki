...
- 2026-07-24 本地 MCP v0.5 的批准基线、协议核心、控制合同和 Web 系统设置集成已按明确边界形成 3 个提交并快进合入本地 `main`，运行代码 HEAD 为 `4087b5e`，尚未 push。合并态 MCP `99` 项测试、快速工程门禁、5173 内容/API smoke 与真实“启动 → ready → 停止”闭环通过；5173 由独立 `main` worktree 提供，稳定开发 profile 仍显示 `5` 条成熟度测试项目，28775 已释放。完整 pre-commit 仍被 `main` 原有的 `5` 个搜索质量探针阻断，本轮未把其他分支中的搜索修订混入 MCP 提交。原 `codex/windows-electron` 脏工作区及其中 Windows、Dashboard、成熟度、App 和数据修改未纳入 MCP 提交；T3、D0-Pilot、M1、真实数据、真实用户数据、客户端配置、系统信任库和 packaging 仍未授权。
