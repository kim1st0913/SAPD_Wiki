...
Delivery Bundle 当前 macOS 交付物按 DMG 验收：2026-07-06 已重打 `apps/macos/SAPDWiki/dist/SAPD-Wiki-mac-arm64.dmg`，SHA-256 `5e78d0d04e6165ce06254d406490317b84273877e5d37dd6bac49812e27d7ce0`，并从 DMG 挂载后的 `.app` 验证 OI-157 批注 Markdown 导出、诊断脚本、`user_schema_0.3` 和 runtime API。旧 ZIP alpha 设计入口仍为 `docs/09-delivery/zip-bundle-1.0-alpha-design.md`，但不再作为当前 macOS 版本验收口径；既有 alpha 试发材料仍在 `/Users/kim1st/Documents/kim note/04_workspace/research/知识库工程/sapd wiki bundle/dist/releases/0.1.0-alpha/`。Windows 构建脚本和验收清单已就绪，但 Windows 原生 `SAPD-Wiki-Backend.exe` 仍需 Windows x64 环境实测；release manifest 中 Windows 保持 `pending / not_verified`。如需发布本轮 DMG，必须先确认复制范围、checksum 和 release manifest 更新。
