...
- 2026-07-07 最新 macOS DMG 已按正式版本 `0.1.5` 重新打双包：授权版为 `apps/macos/SAPDWiki/dist/license/SAPD-Wiki-0.1.5-license-20260707-050440Z-mac-arm64.dmg`，SHA-256 `4010b3882b79a0dfa04f171404c15543ad6ca1dc1579c4f9e115953e0d45cfca`；无授权版为 `apps/macos/SAPDWiki/dist/no-license/SAPD-Wiki-0.1.5-no-license-20260707-050440Z-mac-arm64.dmg`，SHA-256 `a9c42840571091df2e0950011e867dcaf2553c33d0d300d5d79a4f6a56b0f59d`；两包大小均为 `164M`。本轮固化 macOS 双变体交付：`package_dmg.sh` 默认一次生成 `license` / `no-license` 两个 DMG，版本号和时间戳一致，文件名和存储路径区分变体。初始化路径规则改为用户选择父级保存位置，App 自动创建 `SAPDWiki` 子目录，`Runtime` 和默认 `export` 均位于该目录下。授权版启动先弹授权窗口，正确授权码 `Passc0de` 后显示 `已激活` 且不受限制；不知道授权码可跳过进入 30 天试用；到期后不能跳过。无授权版不弹授权窗口，前端授权状态显示 `无限制版`。`Info.plist` 已统一为 `CFBundleShortVersionString=0.1.5`、`SAPDWikiDisplayVersion=0.1.5`，并通过 `SAPDWikiLicenseMode=license/no-license` 区分变体。该 DMG 为 ad-hoc signed 非公证包，外部分发仍可能触发 macOS `隐私与安全性` 手动允许打开。
