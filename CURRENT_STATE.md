...
- 2026-07-07 最新 macOS DMG 已按正式版本 `0.1.6` 重新打双包：授权版为 `apps/macos/SAPDWiki/dist/license/SAPD-Wiki-0.1.6-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `f2dba825f488568c68aa421a13c85244265f216725d3fac4108cbd74c26f215d`；无授权版为 `apps/macos/SAPDWiki/dist/no-license/SAPD-Wiki-0.1.6-no-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `c3f14793b9aa66fff51a7fcbab7d0e5b2ca2105e81bd790f85c3e168b124b4a3`；两包大小均为 `164M`。本轮版本升级规则：小版本从 `0.1.5` 升至 `0.1.6`，`README-FIRST.md` 外层和 Runtime 内层均增加 `Changelog`，记录双包生成、父级保存位置自动创建 `SAPDWiki/Runtime` 与 `SAPDWiki/export` 等变化。`package_dmg.sh` 继续默认一次生成 `license` / `no-license` 两个 DMG，版本号和时间戳一致，文件名和存储路径区分变体。授权版启动先弹授权窗口，正确授权码 `Passc0de` 后显示 `已激活` 且不受限制；不知道授权码可跳过进入 30 天试用；到期后不能跳过。无授权版不弹授权窗口，前端授权状态显示 `无限制版`。`Info.plist` 已统一为 `CFBundleShortVersionString=0.1.6`、`SAPDWikiDisplayVersion=0.1.6`，并通过 `SAPDWikiLicenseMode=license/no-license` 区分变体。该 DMG 为 ad-hoc signed 非公证包，外部分发仍可能触发 macOS `隐私与安全性` 手动允许打开。用户已否决正式 Developer ID 签名公证方案和安装 / 自动更新机制方案，按内测交付现状接受并归档 `OI-190`。
...
Delivery Bundle 当前 macOS 交付物按 DMG 验收：2026-07-07 已按正式版本 `0.1.6` 重打双变体 DMG，授权版 `apps/macos/SAPDWiki/dist/license/SAPD-Wiki-0.1.6-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `f2dba825f488568c68aa421a13c85244265f216725d3fac4108cbd74c26f215d`；无授权版 `apps/macos/SAPDWiki/dist/no-license/SAPD-Wiki-0.1.6-no-license-20260707-085841Z-mac-arm64.dmg`，SHA-256 `c3f14793b9aa66fff51a7fcbab7d0e5b2ca2105e81bd790f85c3e168b124b4a3`；两包均为 `164M`。已验证两包 `hdiutil verify`、两套 staging App `codesign --verify --deep --strict`、`Info.plist` 版本键和 `SAPDWikiLicenseMode`、包内用户库 `user_notes=0`、DMG 根目录中文 `README-FIRST.md`、Runtime 中文 `README-FIRST.md`、两层 README 均含 `Changelog`、前端首页轻量 smoke、初始化路径 `<所选父级保存位置>/SAPDWiki/Runtime` 和默认下载路径 `<所选父级保存位置>/SAPDWiki/export`。授权规则：授权版 macOS 壳层启动前先检查授权状态，未激活时显示授权窗口，试用未到期可跳过，到期后禁止跳过；正确授权码激活后不受限制。无授权版不显示授权输入窗口，不启用 30 天倒计时。前端顶栏从 `/api/v1/health.license` 读取授权状态并替换原 `!` 入口。红色关闭按钮隐藏主窗口并把 App 退到后台；Dock / 菜单重开和最小化恢复统一走主窗口恢复函数，避免 WebView 隐藏或不重绘后只剩黑屏。旧 ZIP alpha 设计入口仍为 `docs/09-delivery/zip-bundle-1.0-alpha-design.md`，但不再作为当前 macOS 版本验收口径；既有 alpha 试发材料仍在 `/Users/kim1st/Documents/kim note/04_workspace/research/知识库工程/sapd wiki bundle/dist/releases/0.1.0-alpha/`。Windows 构建脚本和验收清单已就绪，但 Windows 原生 `SAPD-Wiki-Backend.exe` 仍需 Windows x64 环境实测；release manifest 中 Windows 保持 `pending / not_verified`。如需发布本轮 DMG，必须先确认复制范围、checksum 和 release manifest 更新。
