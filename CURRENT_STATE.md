...
- 2026-07-23 用户已确认按 Web 优先 v0.2 执行批次 B `B0–B5`。既有批次 A 校准为 synthetic-only 核心、协议、控制合同、Web 骨架和 fail-closed 平台边界通过；当前从 B0 干净集成基线开始，只在稳定 `http://127.0.0.1:5173/` 完成真实 Sidecar、loopback HTTPS、本地 OAuth、授权确认、五项 Tool、真实状态页面和自有测试客户端端到端。B5 PASS 后停止，当前电脑/当前 Codex 真实验证仍需 C1 单独授权；macOS/Windows 生产运行时、packaging、D0、真实/用户数据和 push 均不在批次 B。现行计划为 `docs/06-implementation/local-mcp-web-first-development-and-client-validation-plan-v0.2.md`，授权记录为 `docs/07-governance/approvals/local-mcp-batch-b-web-development-approval-2026-07-23.md`，跟踪为 `OI-199`。
