...
- 2026-07-23 本地 MCP 产品开发计划 v0.1 的批次 A 已在独立 `codex/local-mcp-product-development` 分支完成并通过 D6：正式只读知识服务、Sidecar、控制 API、Web“系统设置 → AI 集成”、Web synthetic 启停、macOS/Windows Supervisor 与桥接均已实现。T0–T2 `53/53`、D1 `24/24`、D2 `19/19`、D3/Web `24/24`、集成 `3/3`、签名 `7/7`、Electron `27/27`、Swift `17/17` 通过，macOS 完整构建通过；浏览器宽/窄屏六区、synthetic 启停、桌面能力门禁和零 console error 验收通过。原 dirty checkout 保持只读；真实/用户数据、真实客户端配置、系统信任、真实 OAuth、D0、packaging 和 push 均为 0。当前停止在批次 A 边界，下一步 L1 用户本机当前客户端验证需单独授权；C1 与 R1 继续后置，跟踪为 `OI-199`。
