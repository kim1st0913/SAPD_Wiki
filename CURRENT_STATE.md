...
- 2026-07-23 本地 MCP v0.5 已获 `PASS`，用户已明确授权在独立 `codex/local-mcp-m0t` worktree 串行执行 G0 与 `T0 → T1 → T2`。本轮仅允许机器合同、synthetic fixture、只读 Runtime probe、隔离 loopback HTTPS / OAuth / Tool harness 和本地阶段提交；不授权 push、T3、D0-Pilot、M1、真实数据、用户数据、App integration 或 packaging。G0 正在建立审批记录与纯文档基线，跟踪为 `OI-199`。
