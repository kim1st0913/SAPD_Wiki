...
面向咨询顾问的 V1 交付目标仍是本地可运行应用：顾问解压后第一次打开，系统可部署预置 SQLite 数据库、页面数据包和预览资源，然后直接进入知识库工作台。顾问端不需要自行导入资料，不执行 ETL 或 migration；第一期也不做登录、注册、账号和权限体系。详细交付模型见 `docs/01-architecture/consultant-delivery-model.md`。
