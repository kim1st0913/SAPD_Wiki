...
function ensureConfiguredSettings() {
  ...
  dialog.showMessageBoxSync(mainWindow, {
    type: "info",
    title: `${APP_NAME} 首次启动`,
    message: "请选择 App 保存位置",
    detail: "应用会在所选位置创建 SAPDWiki 文件夹，并分别管理文件上传路径、文件下载路径和 Runtime。程序安装位置与这里的数据位置相互独立。",
    buttons: ["选择 App 保存位置"],
  });
  const selected = chooseDirectory(
    "选择 App 保存位置",
    app.getPath("documents"),
    "请选择 App 保存位置，应用会在其中创建 SAPDWiki 文件夹。",
  );
  if (!selected) {
...
function settingsSnapshot(extra = {}) {
...
function chooseSettingsDirectory(kind) {
  if (!currentSettings) return settingsSnapshot({ available: false });
  const options = {
    dataRoot: {
      title: "更改 App 保存位置",
    ...
    importDirectory: {
      ...
      message: "选择成熟度模板、评分等文件的上传路径。",
    },
    downloadDirectory: {
      title: "更改文件下载路径",
      defaultPath: currentSettings.downloadDirectory,
      message: "选择报告、评分、问题和诊断文件的下载路径。",
    },
  };
  const option = options[kind];
  if (!option) throw new Error(`Unsupported settings directory kind: ${kind}`);
  const selected = chooseDirectory(option.title, option.defaultPath, option.message);
  if (!selected) return settingsSnapshot({ cancelled: true });
  const nextSettings = kind === "dataRoot"
  ...
  if (!changed) return settingsSnapshot();
  currentSettings = nextSettings;
  ensureSettingsDirectories(currentSettings);
  saveSettings(currentSettings);
  createApplicationMenu();
  return settingsSnapshot({ changed: true, restartRequired: true });
}

function changeSettingsDirectory(kind) {
  const result = chooseSettingsDirectory(kind);
...
function showApplicationSettings() {
  ...
  const response = dialog.showMessageBoxSync(mainWindow, {
    ...
    title: "SAPD Wiki 设置",
    message: "本地文件夹",
    detail: `App 保存位置：${currentSettings.dataRoot}\n\n文件上传路径：${currentSettings.importDirectory}\n\n文件下载路径：${currentSettings.downloadDirectory}\n\nRuntime：${path.join(currentSettings.dataRoot, "Runtime")}`,
    buttons: ["更改 App 保存位置", "更改文件上传路径", "更改文件下载路径", "关闭"],
    cancelId: 3,
  });
  ...
  const template = [
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        { label: "打开 App 保存位置", enabled: Boolean(currentSettings), click: () => shell.openPath(currentSettings.dataRoot) },
        { label: "打开文件上传路径", enabled: Boolean(currentSettings), click: () => shell.openPath(currentSettings.importDirectory) },
        { label: "打开文件下载路径", enabled: Boolean(currentSettings), click: () => shell.openPath(currentSettings.downloadDirectory) },
