...
### 9.2 必须在 M0-T 决定的 TLS 终止方案

可实验但只能冻结一种正式方案：

| 方案 | 说明 | 主要门禁 |
|---|---|---|
| 加密 PKCS#8（PEM 编码）私钥文件（优先比较） | Sidecar 使用 `SSLContext.load_cert_chain(..., password=...)` 解密当前用户最小 ACL 的私钥文件；随机口令保存在 Keychain/DPAPI，只在进程内存或受认证私有通道中交付 | 私钥始终加密落盘；口令不得进入命令行、环境变量、日志、设置或诊断包；验证轮换、备份、崩溃恢复、卸载和打包 |
...
Python 标准 `SSLContext.load_cert_chain` 接受 `password` 参数，可读取加密私钥文件；因此“加密私钥文件 + 平台安全存储口令”是符合当前威胁边界的务实候选。但它只保护静态文件与离线副本，不抵御已完全控制当前用户会话或 Sidecar 进程内存的攻击者。`M0-T` 必须与另外三种方案比较跨平台实现、打包、升级和恢复成本，不能因 API 可调用就直接冻结。
...
2. **连接 Codex**：主动作、客户端选择、配置引导、服务 URL 次动作。
3. **授权客户端**：身份状态、scope、授权/使用时间、策略版本、逐个撤销。
4. **数据访问**：当前开放范围、明确排除项、对象/字段白名单。
5. **隐私与审计**：审计状态、保留期、查看、导出、清除、关闭。
6. **诊断与重置**：TLS、OAuth、Runtime、策略、端口、日志、脱敏诊断、重置 AI 集成。
...
- TLS 首装、拒绝、修复、轮换、重置和卸载的隔离实验；
- 加密 PKCS#8（PEM 编码）、App 终止、签名 Helper 和原生密钥 Provider 的决策记录；
