# 剩余 21 个 Excel Sheet 建模草案

本文档用于在写代码前，对 `wiki sample.xlsx` 中尚未进入第一批 ETL 的 21 个 Sheet 做建模。它是需求调整和字段讨论的工作底稿，不是最终实现承诺。
...
剩余 21 个 Sheet 的价值不是再造一套主链路，而是围绕现有能力体系补充 4 类维度：
...
| B. 环境作用域补充 | `信息化环境-信息化对象-安全作用域映射` | P1 | 可补强当前场景/对象/作用域模型 |
| C. 安全工作/流程/职能 | `安全能力-安全工作`、`安全能力-安全管理元素（high level）`、`安全能力-安全管理元素（细化版本）`、`安全职能流程清单（完善L4）`、`安全工作职能清单`、`gartner工作岗位参考` | P1 | 第二批最适合先做，能直接扩展能力详情页 |
| D. 生命周期 | `LC-DT 数据生命周期`、`LC-DT 数据生命周期场景目录`、`LC-AP 应用安全开发生命周期`、`LC-AP 应用安全开发生命周期元素目录` | P2 | 结构独立，适合做生命周期专题页 |
| E. 标准/框架/控制项 | `安全能力-网络安全制度、框架映射`、`等保三级测评清单`、`CSF2.0`、`27001-2022`、`DSP2级策略清单（2024年版本）`、`CIS CSC V8`、`CRF`、`NIST 800-53rev5` | P2 | 数据量大，建议在流程/职能稳定后做 |
...
| 4B 标准控制项主数据 | `等保三级测评清单`、`CSF2.0`、`27001-2022`、`DSP2级策略清单（2024年版本）`、`CIS CSC V8`、`CRF`、`NIST 800-53rev5` | 再建立每套标准自己的控制项对象 |
...
### 6.3 `信息化环境-信息化对象-安全作用域映射`

| 项 | 草案 |
|---|---|
| 每行代表 | 信息化环境中的信息化对象与作用域的映射 |
| 复用对象 | `information_environment`、`information_object`、`scope_type` |
| 新对象 | 暂无 |
| 字段 | 环境、对象、作用域列表 |
| 关系 | `information_object applies_to_scope scope_type`、`information_object belongs_to information_environment` |
| 当前策略 | P1，但它和已实现的场景映射 Sheet 部分重叠。进入 ETL 前要确认是否作为补充来源，还是替代/校验来源。 |
...
| 每行代表 | 一个等保三级控制要求 |
| 新对象 | `standard_framework`、`standard_control` |
| 字段 | 等级保护、等保要求、等保控制项、等保三级控制要求、DSP安全策略项 |
...
| 关系 | 策略项属于 DSP2 级框架；可与等保等控制项建立候选映射 |
| 当前策略 | 第四批 4B。建议先作为 `policy_item`，避免和标准控制项混淆。 |
...
| 每行代表 | 一个 CRF 保障措施 |
| 新对象 | `standard_control` |
| 字段 | 保障措施分类、保障措施系统、CRF成熟度等级、保障措施ID、保障措施描述 |
| 关系 | 保障措施属于 CRF 框架、分类和系统 |
| 当前策略 | 第四批 4B |
