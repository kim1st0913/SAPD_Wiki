...
> 归档状态：`historical / superseded delivery architecture`

> 状态：`historical / architecture evaluation`
>
> 当前桌面交付流程见
> `docs/09-delivery/desktop-packaging-runbook.md`。本文只保留早期预构建数据库和 ZIP
> alpha 的架构评估，不再作为工程实施入口。
