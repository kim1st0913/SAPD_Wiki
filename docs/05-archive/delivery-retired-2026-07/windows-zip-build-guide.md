...
> 状态：`retired / historical ZIP and backend-only workflow`
>
> 当前 Windows `Setup.exe` 生产流程见
> [`desktop-packaging-runbook.md`](desktop-packaging-runbook.md)。下文只保留旧 ZIP /
> backend-only 流程的历史证据，不得用于当前发布。
