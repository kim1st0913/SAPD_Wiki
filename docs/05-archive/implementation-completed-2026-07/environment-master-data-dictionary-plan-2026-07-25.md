...
> 归档状态：`completed / P0-P8 execution evidence`
...
| P7 受控切换与观察（已完成） | 开启字典能力开关；清理运行时缓存；Web、搜索、Dashboard、批注/收藏定位回归 | shared runtime | 观察期无回退触发；旧树 fallback 仍保留 |
| P8 发布回归（Web已完成） | Web共享运行时、离线数据包和导出回归；App Runtime按新包后置 | 不自动构建 DMG | Web门禁通过；当前源码App/DMG证据仍按发布矩阵另行完成 |
...
P0—P7已经完成。P6在停止5173与MCP写者、取得单写者锁并生成独立恢复包后，对正式基础库执行P2冻结计划：首次回填61个编号、新增16个 `environment_segment_type`、29条 `instance_of`、58条来源引用和106条迁移审计，重复执行新增全部为0；正式状态为10/16/51条主数据、29/67条上下文，旧7757条关系及保护文件保持。P6报告和恢复包位于 `data/exports/worker-verify/plan-env-md/p6-20260726T015418Z/`，含169个恢复文件、基础库和用户库热备份，均已完成完整性、外键与临时恢复验证。正式字典包与P4影子包字节一致；旧环境树、拆分投影、底图、Dashboard、源Excel及用户库未修改。

P7经用户单独授权后将 `environmentMasterDictionary` 开启，并重启稳定5173清理运行态。主数据字典10/16/51、29/67关系上下文、106条用法关系、旧目录fallback、Dashboard 10/51/6口径、环境映射深链以及批注/收藏只读引用均通过；正式库、用户库和保护包哈希未变化。P7报告位于 `data/exports/worker-verify/plan-env-md/p7-20260726T022354Z/`，门禁为 `ready_for_p8_web_release_regression`。

P7.1根据用户追加要求完成定义与投影修正。环境子类定义的权威链确认为P2人工语义裁定，原始Excel没有定义列；同一方法形成 `environment-and-object-definition-adjudication.p7-1.json`，补齐10条信息化环境和51条信息化对象定义。临时副本首次更新61、重复更新0且独立恢复通过；正式写入前备份基础库、用户库和公开字典包，正式库只更新61个空 `description` 并新增61条确定性审计。主数据身份、编号、29/67原关系和来源证据未变。环境关联使用从10条环境自身占位改为29条真实子类上下文，当前三类关联使用为29/29/67共125条，逐环境展开数为3/5/3/1/4/4/2/1/3/3。证据与恢复包位于 `data/exports/worker-verify/plan-env-md/p7-1-20260726T051754Z/`。
...
P0—P8 Web门禁已经完成，当前停止在新包 App Runtime / DMG 发布矩阵与人工 UAT。P7开关保持true；如出现第10.3节任一触发条件，先将开关恢复为false并重启稳定5173，只有数据或引用证据异常时才按P6恢复包执行更深层回退。系统Chrome、当前源码App Runtime和DMG尚未验收，5173 Web结论不得替代这些证据。
