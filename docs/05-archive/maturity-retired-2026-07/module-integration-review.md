...
2026-07-09 已同步 Product Design 口径，成熟度能力在产品信息架构中按用途拆分：

| 入口 | 承载内容 | 边界 |
|---|---|---|
| `安全指南` | 成熟度模型使用指南、评分方法说明、等级解释、模板填写说明、评估流程说明 | 只承载说明类和方法论阅读内容，不承载客户评估任务和评分结果 |
| `工作台 > SAPD 成熟度评估` | 评估任务、填报会话、证据导入、匹配审查、评分结果、差距建议、报告导出 | 只承载评估工作流和客户运行数据，不维护能力主数据 |
| `知识库字典 > 安全能力清单` | 安全能力、能力关注点及相关权威字典数据 | 是 maturity 模块引用能力数据的权威来源，maturity 只读引用，不复制一套能力主数据 |

后续原型和代码实现必须遵守该拆分，不能把说明类内容、评估工作流和能力权威数据混成一个 maturity 总页面。
...
- 用 `知识库字典 > 安全能力清单` 的权威数据生成 `Reference_Capabilities`；
- 用现有关系做匹配扩展；
