...
> 归档状态：`historical / full roadmap before documentation governance round 4`。
...
- macOS 从正式 Mac 主工作区本地生成 DMG。
- Windows 从公开 `main` 精确 SHA 和私有 Delivery Data 生成 `Setup.exe`。
- 安装包只携带受控基础数据、内容资产和空用户库模板。

范围：

- macOS Swift / WKWebView 壳与 DMG；
- Windows Electron / NSIS 壳与私有 Runner；
- 只读基础库、内容资产库、页面数据包和预览资源；
- 应用数据目录部署；
- 备份恢复说明；
- 用户指南与发布验收矩阵。
...
- 普通用户不安装 Python / Node / SQLite CLI；
- 普通用户不执行 ETL、migration 或数据库选择；
- 真实用户数据库不进入安装包或 GitHub；
- 导入、清洗、审批、校验和发布构建只在内部维护流程完成。

当前入口：`docs/09-delivery/desktop-packaging-runbook.md`。
