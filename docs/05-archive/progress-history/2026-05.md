...
## 2026-05-27 根目录 `progress.md` 瘦身补充

- 根目录 `progress.md` 达到 120 行轻量阈值，本轮先归档瘦身再继续处理开发体验问题。
- 近期关键状态：安全能力映射页加载慢已通过 `/api/v1/capabilities/workspace-initial` 和按关注点 `workspace-projection?focus_id=...` 收口，首屏业务请求降到约 `204KB`；刷新状态恢复已覆盖当前路由、选中对象、目录展开、维护页 Tab、指南页内容和 LC-AP / LC-DT 选择。
- 近期关键修复：`OI-085` 关注点小投影加载中状态、`OI-086` 无服务矩阵计数、`OI-087` 管理 high level `/` 空值继承、`OI-088` 标准 / 框架空态样式均已修复。
- 本轮继续新增 `OI-089`：前端验证频繁弹出 Chrome 意外退出且 `5173` 刷新不稳定，处理范围限定为本地验证脚本、固定预览服务缓存头和端口守护脚本。
