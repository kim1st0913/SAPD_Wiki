...
## 2026-06-07 Progress 瘦身前摘要

- 2026-06-05 至 2026-06-07 根目录 `progress.md` 曾保留 `OI-128C` 批注基线、`analytics_summary`、`OI-135 / DB-11 / DB-2`、`OI-136 / FE-ROUTE`、`OI-133 / ArchiMate` 和工作台 / 数据篮 API 的长验证清单。
- 2026-06-07 主控按轻量恢复规则再次瘦身根目录 `progress.md`：根目录只保留当前状态、最近关键事项、当前问题索引和最近验证摘要；完整长过程继续以本归档文件作为历史入口。
- 2026-06-07 新增会话卡顿交接治理：只有用户明确说“当前会话卡顿，需要交接”或等价表达时，Codex 才能停止当前会话执行、整理交接包、创建同名递增新线程、写入新线程初始提示并在当前会话最终回复后归档；没有明确卡顿交接请求时，不得自动创建新线程。
...
## 2026-06-09 根目录 progress 瘦身摘要

- 根目录 `progress.md` 在 Environment Basemap Lab 系列任务后达到 120 行阈值，已再次瘦身为轻量恢复入口；瘦身前最近事项包括 dashboard 数据统计方案、批注抽屉锁定、ArchiMate 页面修复、信息化环境底图 HTML renderer / SVG Lab / node-details / Detail Projection 等长记录。
- Environment Basemap Lab 当前关键状态：实验页 `/environment-basemap-lab` 使用官方 `environmentBasemap.svg` 作为视觉层，只加载 `environmentBasemap.svg`、`environmentBasemap.semantic.json`、`environmentBasemap.node-details.json`；`semantic nodes=97`、`edges=73`、`bound=91`、`ignored=6`、`detailReadyNodes=91`、`missingDetailNodes=0`。
- Lab 已完成：官方 SVG 可见、透明 hit layer、分类型详情投影、节点旁浮层、Esc / 空白关闭、适应屏幕 / 缩放 / 还原 / 全屏、异常 SVG bbox 回退到 semantic contentBounds。

## 2026-06-12 根目录 progress 瘦身摘要

- 根目录 `progress.md` 在 Scope-Service-Module Mapping Reimport 1.0-1.5、正式数据包替换、临时核对表和多轮 Excel 数据修正后再次超过 120 行阈值，已瘦身为轻量恢复入口；详细命令、哈希和阶段性验证仍以本归档和 `data/exports/worker-verify/` 产物为准。
- 当前环境映射数据线最近完成状态：原始 `data/raw-samples/wiki sample.xlsx` 的 `作用域-安全技术服务-安全技术模块映射` Sheet 最近保存到 2026-06-12 16:58 后，已完成审计、candidate、replacement bundle、正式 `environment-workbench.json`、正式 `environmentBasemap.node-details.json` 和临时核对表刷新；最近统计为 `mergedRangeCount=417`、`normalizedRowCount=773`、`objectScope=156`、`objectService=731`、`serviceModule=640`、`serviceMeasure=123`、`moduleSystem=214`、`scopeCompletenessIssueCount=0`、`duplicateInformationObjectContextCount=0`、`sameNameDifferentContextCount=8`。
- 最近正式数据哈希：`frontend/capability-browser/public/data/environment-workbench.json` 为 `e7ca633b1ef64e7db9d6eda845518a3dd4d8d4bd3e9c8ce844efe119c8d25789`；`frontend/capability-browser/generated/environmentBasemap.node-details.json` 为 `a062682b92228d0a623207dfdd0a1dd5c42cb4f98a83bbd188a1126972836a16`。
- 临时核对表最近状态：`frontend/capability-browser/public/data/review/environment-manual-review-checklist.json` 为 `67` 个上下文、`773` 行、`highRiskContextCount=15`、`mediumRiskContextCount=3`、`duplicateServiceRowCount=59`、`scopeCompletenessIssueContextCount=0`；操作系统类上下文中仅 `工业控制网络||L3层||L3主机操作系统` 仍残留重复服务。
- 本轮数据治理新增 `scripts/apply_scope_supplements_to_source_excel.py`，已将 `service_scope_reverse_check` 补出的作用域写回原始 Excel E 列合并单元格左上角，共更新 `32` 个 E 列合并单元格且 `eColumnMergedRangesPreserved=true`；备份位于 `data/exports/worker-verify/source-excel-backup/wiki sample.before-scope-writeback.20260612-155248.xlsx`。
- 临时核对 UI 当前位于 `/environment-mapping` 的 `数据核对（临时）` Tab；右侧证据面板已删除，表格主体占满核对区，仍保留行级证据列用于人工核对。当前仍建议先继续清理重复服务数据，再进入正式 UI 字段对齐。
...
## 2026-06-29 根目录 progress 瘦身摘要

- 根目录 `progress.md` 在信息化环境对象关系图多轮页面调整、demo 设计和新主控交接后达到 240 行，已再次瘦身为轻量恢复入口。
- 2026-06-24 至 2026-06-25 对象关系图正式页经历中轴图迁入、作用域颜色图例、模块 / 措施颜色语义、服务侧对齐、HUD 宽度与图例对齐、滚动重绘稳定性等多轮修正；最终正式页保持 `environment-object-graph-stage / environment-object-graph-links / environment-object-graph-hud / environment-object-graph-cluster` 中轴结构，未修改原始 Excel、SQLite、正式数据包、node-details、review checklist、标准包或生命周期数据包。
- 2026-06-28 至 2026-06-29 新增并优化 demo：`frontend/capability-browser/environment-object-apple-shell-demo.html` 和 `frontend/capability-browser/environment-object-vertical-flow-demo.html`。主 demo 已包含 `当前布局`、`汇聚布局试验`、`纵向流试验` 三个 tab；横向当前布局与汇聚布局均有 `routing spine` 视觉路由带，纵向流 demo 也保留同类关系语义。
- demo 关系语义已固化：青蓝实线表示 `安全技术服务 -> 安全技术模块`，琥珀虚线表示 `安全技术服务 -> 安全技术措施`，灰蓝点虚线表示 `安全技术模块 / 措施 -> 安全系统`；不得绘制 `安全技术模块 -> 安全技术措施` 关系。
- 用户明确不喜欢 `2 映射 / 3 映射` 文案，demo 已改成 `1 模块 / 1 措施`、`2 模块 / 1 措施` 等业务表达。
- 最新 checkpoint 已提交并推送：`d69063a Checkpoint environment object demos`。提交范围为两个 demo 文件和当时的 `progress.md`；交接时 `main...origin/main` 同步、工作区干净、5173 项目服务正常。
- 新主控接手后，用户明确要求进入代码级迁移：新增 `信息化环境-安全技术备份页`，将 demo `汇聚布局试验` 的映射部分迁移到正式 `信息化环境-安全技术` 页；截图中的对象摘要区保留原样，只替换页面映射部分。

## 2026-06-30 根目录 progress 轻量瘦身摘要

- 根目录 `progress.md` 在 OI-149 与批注治理并行记录后达到 123 行，已将 2026-06-24 至 2026-06-29 的环境对象图 / demo 长记录折叠为一条恢复索引，详细历史仍以本归档为准。
- 保留的当前结论：正式环境对象图中轴结构仍是可回退基线；demo 迁移或后续页面迁移必须明确保留对象摘要区边界；环境安全技术关系语义仍为 `安全技术服务 -> 安全技术模块`、`安全技术服务 -> 安全技术措施`、`安全技术模块 / 措施 -> 安全系统`，不得新增 `安全技术模块 -> 安全技术措施`。
- 追加瘦身：根目录 `progress.md` 在 OI-149 post-apply 门禁补齐后再次超过 120 行，已将 2026-06-30 macOS App / DMG 支线的三条长记录折叠为一条索引。详细内容包括 SwiftPM AppKit wrapper、PyInstaller onedir Runtime、能力页 API 补齐、Runtime 指纹复用、quarantine 清理、ad-hoc 签名 / Gatekeeper 边界和 DMG 校验；该支线不影响 OI-149 正式数据包治理。
