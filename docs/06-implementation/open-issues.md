...
| 编号 | 状态 | 标题 |
|---|---|---|
| OI-199 | 批次 B PASS / 等待 C1 授权 | 本地 MCP 产品开发与客户端验证 |
...
- 状态：批次 B PASS / 等待 C1 授权
- 严重性：高
- 类型：架构 / 安全 / MCP / 隔离验证 / 治理
- 对象或页面：四份 MCP 机器合同、正式只读知识服务、Sidecar、控制 API、5173 AI 功能集成页面、当前 Codex 联调、macOS/Windows 运行时、打包和客户端兼容矩阵。
- 现象：批次 A 曾只完成 synthetic 核心、协议、控制合同、Web 骨架和平台 fail-closed 边界；批次 B 已补齐 5173 真实 Sidecar、授权和自有客户端闭环，但当前 Codex、桌面生产运行时与实包尚未验证。
- 影响：若直接进入桌面或打包，会在 Web 协议、授权和页面状态尚未闭环时放大平台差异，且可能把平台模拟测试误当成真实可用证据。
- 建单理由：涉及跨模块架构、安全、认证、数据边界和后续 App / packaging 门禁，必须保留阶段证据与授权边界。
- 当前处理：批次 B `B0–B5` 已在独立分支通过，5173 保持隔离预览且 MCP 停止；执行停止在 B5，等待 C1 单独授权，未 push。
- 需要确认：C1 真实 Codex 配置、系统信任、配置/凭据快照与结束后的恢复或保留范围仍需单独确认。
- 验收入口：`docs/06-implementation/local-mcp-b5-web-real-loop-pass-report-2026-07-23.md`。
- 关闭条件：B5 Web 真实闭环、C1 当前 Codex、D/E 桌面与实包、F 兼容矩阵及 D0/发布门禁按各自授权完成。
- 修复说明：v0.1 和批次 A 保留为 synthetic 历史基线；v0.2 批次 B 已实现真实 Web 控制面、Sidecar、TLS/OAuth、授权队列、五项 Tool E2E 和零用户库文件的隔离 Web 模式。完整 OIDC 在 B2 决定不启用。
- 验证结果：自动化 `89/89`，应用内浏览器宽/窄屏、键盘刷新、返回、真实启停和 console 门禁通过；实现提交为 `aa51f78`。真实 Codex、系统信任、桌面生产接线、packaging、D0、数据和远程写入仍为 0。
