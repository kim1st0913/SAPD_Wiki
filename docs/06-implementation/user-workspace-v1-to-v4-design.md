...
2026-06-04 后续 UI 主入口已升级为右侧浮层批注抽屉与工作台，正式设计见 `docs/06-implementation/workspace-annotation-and-capability-remix-design.md`。本文继续保留用户库分阶段能力、数据表和 API 路线；横向 `加入关注清单 / 收藏备注` 条不再作为后续实现基准。
...
`OI-128C` 的目标边界：下线横向收藏 / 备注条，改为右侧浮层批注抽屉；`收藏` 不再作为主业务动作，后续主语义改为 `批注`、`待复核`、`数据篮` 和 `工作台`。
