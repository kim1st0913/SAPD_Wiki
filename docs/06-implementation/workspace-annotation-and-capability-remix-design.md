...
批注模块的全局需求、颜色基准、页面级回归矩阵和 `OI-128C` 关闭条件以 `docs/06-implementation/global-annotation-requirements-and-regression-matrix.md` 为准。本文只说明工作台方向和用户库路线；任何实现不得只按当前页面局部修复批注。
...
- 用户库治理单独进入 `OI-135 / DB-11`，用于处理 `user_notes` 正式化、旧 `user_favorites` 兼容、测试数据清理、备份 / 恢复、迁移和 base/user read model 合并边界。
...
- 原知识库页面继续作为只读业务浏览层，不承担批注保存、状态、抽屉或定位业务。
- 页面只需要暴露 `data-annotation-value`、`data-copy-text`、`data-annotation-target-ref` 等稳定锚点。
- 批注层统一处理右键添加、抽屉显示、保存、编辑、删除、状态修改、常驻高亮、定位高亮和悬停完整值。
- 如果未来替换批注模块，原页面的数据加载、搜索、表格滚动、复制和只读展示不应被破坏。
...
- 抽屉宽度必须固定在上限内，不允许被长标题、长编码或长批注内容向右撑开；标题、编码和锚点摘要采用单行截断，悬停时显示完整文本。
- 抽屉从右侧滑入，不挤压主页面。
...
- 抽屉头部不重复页面标题、对象标题和自动标签，只保留“批注”、当前页批注数量和一个紧凑锚点栏。
- 当前页只显示一个“当前上下文批注”清单；上下文可以是页面、对象、行或字段，不再同时展示“当前对象批注 / 当前页面批注”两个空列表。
- 批注数量只统计当前页面，不统计全局所有页面。
- 切换页面、对象或二级页面后，抽屉默认收起。
- 如存在未保存批注草稿，切换前必须提示用户选择“保存并切换 / 放弃并切换 / 继续编辑”。
- 批注除删除和改状态外，必须支持修改正文。
- 搜索框、右侧浮层、表格选中和字段锚点的视觉规则纳入 `frontend-global-design-baseline-2026-05-30.md`，不再各页面单独漂移。
- 批注卡片参考 Office Word 批注栏：默认折叠为摘要行，摘要按状态、批注序号、锚点类型小圆标签和锚点值排列；展开后显示正文、状态下拉和操作；不显示无业务意义的头像或“本地用户”占位。
- 批注卡片操作顺序固定为 `定位 / 编辑 / 删除`；`修改` 文案统一改为 `编辑`；点击编辑、保存、取消或修改状态后必须保持当前抽屉滚动位置和卡片展开状态，不得自动收起批注。
- 原始页面上已批注的行 / 字段 / 值采用类似 Office Word 的低噪声标记：字段 / 值优先使用文字级批注墨水，行级使用左侧细线与极浅底色。点击定位时使用短暂扫光 / 柔和发光动画，不使用粗紫色描边、数量胶囊、角标或整块黄底铺满重复值；重复同名值只能在精确上下文匹配时高亮。
- 已有批注的锚点需要有常驻但低噪声的提示；用户点击批注卡片定位时，高亮范围必须匹配锚点类型：字段 / 值只高亮具体值，行级才高亮整行，页面整体批注只定位页面不高亮整页。
- 抽屉内截断的长标题、长编码、长锚点摘要必须有 Apple shell 风格悬停气泡，不能只依赖浏览器原生 `title`。
...
| 字段级 | 某个表格单元格、关系 chip 或详情字段 | V1B |
| 关系级 | 某个能力 -> 标准条款映射 | V2 |
| 选中文本级 | 指南或文档中的一段文字 | V3 |

全局适用范围：

- `安全能力映射`、`信息化环境安全能力映射`、`LC-AP安全开发生命周期`、`LC-DT数据生命周期安全`、`知识库字典`、`标准 / 框架`、`安全指南` 和后续工作台页面都使用同一批注契约。
- 批注模块不得再做页面级特例补丁。新增页面如有表格、矩阵、chip、详情字段或文档值，都必须先接入共享锚点，再进入页面验收。
- 右键 `添加批注` 的业务含义由锚点决定：点击页面空白是页面批注，点击行是行批注，点击字段 / 值 / chip 是字段 / 值批注；没有明确锚点时不得假装创建值级批注。

锚点定位规则：

- 页面组件必须显式声明可批注业务值，统一使用 `data-annotation-value="true"` 和 `data-copy-text` 暴露字段 / 值锚点。批注模块不得靠猜测页面私有 class 或默认向上回退到整行来伪造值级批注。
- 所有共享关系 chip 必须内建值锚点；页面组件只补齐纯文本字段、场景标题、策略正文、参考数据等不走共享 chip 的业务值。新增页面上线前必须跑 `node scripts/audit_user_annotation_contract.mjs`，确认值级批注不会退化成行批注。
- 普通表格单元格是全局兜底锚点：当 `td` 没有共享 chip 或显式值锚点，且单元格文本不是空值、`/`、`暂无` 或 `待补充` 时，右键应生成字段 / 值批注。表头不生成值批注。
- 批注保存时必须记录页面 route、当前工作区 view、当前目录 / 子页面、当前对象、当前 tab 和表格行列上下文。
- 点击批注项时，系统先切换到记录中的页面和子页面，再恢复目录、tab、对象或幻灯片状态；如果目标位于知识库字典或标准 / 框架的折叠目录中，必须自动展开父级分组；最后滚动到具体行 / 字段 / 值。如果目标位于右侧展开抽屉覆盖区，定位前允许自动收起抽屉，避免遮挡被定位数据。
- 定位不能只按显示文本查找；同名值必须结合所在能力、关注点、tab、表格行和字段位置判断。
- 旧批注如果缺少上下文锚点，不得为了“看起来有反馈”而高亮所有同名值；可以只打开对应页面并等待用户重新建立精确批注。
