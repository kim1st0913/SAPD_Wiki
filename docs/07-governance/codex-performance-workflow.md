...
8. 如果出现多会话并行、子 Agent fan-in 不稳定或 dirty diff 混杂，先执行 `docs/07-governance/execution-line-convergence-workflow.md`，默认只保留一个主控和一条写入主线。

## 共性优先、定向收口

用户调用 Product Design 或 `sapd-wiki-root-cause-qa`，通常是为了避免只修截图表面。执行目标是找到共享 owner 并覆盖同类问题，不是把每个小缺陷自动升级为重设计、长文档同步和全工程发布验收。

默认用一次收敛流程完成：

1. 把用户样例作为首个黄金样例，用一次批量搜索定位共享组件、共享样式、数据 / API 契约或交互规则。
2. 列出使用同一 owner 的页面家族，并用证据判断问题是共性还是局部；不得默认只修当前页，也不得无证据扩大范围。
3. 共性问题只修改共享 owner；禁止在多个路由堆叠重复 CSS、组件分支或页面级业务判断。
4. 修改后批量执行目标语法、专项断言和最小运行态检查；视觉问题默认只检查能证明契约的视口，只有响应式问题才扩展多视口。
5. 将其他活动改动造成的门禁失败单独列出，不从当前任务越权修复，也不把无关失败归因于本轮修复。
...
|---|---|
| 用户报告的黄金样例 | 证明原始现象已消失 |
| 同一 owner 的 2—3 个代表页面 / 对象 | 证明共性修复真实覆盖页面家族 |
| 1 个家族外反例 | 证明没有把规则错误扩散到其他业务粒度 |

同类家族不足三个对象时直接验收全家族。至少一条断言必须针对旧的表面补丁回归形态，而不只是检查 class、文件版本或 smoke 成功。

### 升级到重型流程的条件

只在以下至少一项成立时运行完整套件、长文档联动或发布级证据：

- 修改共享运行基础设施、`dataClient`、ViewModel、API、数据包、ETL 或用户状态；
- 改变长期业务、数据、交互、视觉或验收契约；
- 涉及 App-only、release blocker、打包或正式发布；
- 用户明确要求全工程收口、设计审计、多视口验收或发布验收。

普通文案、显隐、间距和已存在契约下的局部实现缺陷，只记录一条简洁进度，执行目标文件语法、专项断言和一个最小运行态检查。
...
出现以下任一情况时，业务开发暂停，优先收敛执行线：

- 用户明确反馈执行线太多、会话太多或单会话效率下降；
- 子 Agent 失败、无法 fan-in，或多个会话给出互相冲突的状态；
- `git status --short --branch` 显示大量未提交改动，且涉及多个主题；
- 治理脚本失败但当前任务又准备继续改同一风险区域；
- 主控无法一句话说明“当前唯一写入任务是什么”。

收敛时只做三件事：

1. 验收 dirty diff；
2. 暂停或合并执行线；
3. 形成一个可 checkpoint 的下一步。
