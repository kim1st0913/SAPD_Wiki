...
| EL-024 | 安全能力映射数据加载稳定性治理 | 治理中 | `OI-132`；2026-06-03 第一轮已修复能力清单 / 知识库字典 / 标准框架类页面的加载竞态入口：轻量 `capabilityInitial` 不再覆盖完整 `capability-tree`，维护页缺必需包时主动自愈加载，标准 active tab 未加载完成时显示加载态；但安全能力映射四 tab 的对象级空态可信度仍未整体关闭 | 第一轮前端加载稳定性修复已完成；继续安全能力页前仍必须先做只读诊断和审计脚本，不直接改 UI 空态 | 建立加载状态诊断、对象一致性断言、四 tab 回归审计和真实空态 / 加载失败 / fallback 区分规则；下一步优先覆盖 `T-AS.AD-01` 标准 / 框架映射显示与 ViewModel `standardRows=1` 的一致性 |
