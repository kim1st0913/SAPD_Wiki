...
2026-06-06 已新增总 backlog 收敛入口：`docs/07-governance/backlog-convergence-2026-06-06.md`。本文负责记录执行线和线程映射；总 backlog 收敛表负责给所有未开展任务排序，区分当前必须先做、可并行只读、后续开发和后置任务。
...
| EL-023 | Apple Morandi 配色 demo / 全局视觉基准 | 后置 | 线程 `019e72f1-f6f8-7fa0-b9ef-378551251726` 产出颜色系统 demo、`DESIGN.md` 配色基准和 handoff；当前 canonical 入口已统一到 `environment-object-apple-shell-demo.html#color-system`，旧 `apple-morandi-color-demo.html` 仅作为嵌入来源保留 | 用户要求继续全局视觉统一，或页面 Gap Check 需要统一色彩 token 时恢复 | 只作为设计参考，不在 dirty diff 未验收前改 `styles.css` |
