...
| EL-024 | 安全能力映射数据加载稳定性治理 | 待治理 | `OI-132`；用户截图显示 `T-AS.AD-01` 在 `标准 / 框架映射` tab 为 `0 控制项`，但当前页面无法证明这是源数据真实为空、workspace-view 未加载、fallback、对象 mismatch 或重渲染缺失 | EL-001 完成后；若继续安全能力页，必须先做只读诊断和审计脚本，不直接改 UI 空态 | 建立加载状态诊断、对象一致性断言、四 tab 回归审计和真实空态 / 加载失败 / fallback 区分规则 |
