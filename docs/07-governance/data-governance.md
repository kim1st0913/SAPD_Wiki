...
## 3.2.1 Environment Mapping 字典型对象业务规则

`信息化环境 - 信息化对象 - 安全技术服务 - 安全技术模块 / 措施` 是架构师定义的字典型结构。一个信息化对象下已填写的安全技术服务、模块和措施，就是该对象的正式定义。

执行规则：

- 不允许用“某个安全技术服务在全局安全技术模块清单中理论可关联多个模块”反推每个信息化对象必须补齐全部模块。
- 不允许用“某个安全技术模块在全局安全技术模块清单中理论可关联多个服务”反推每个信息化对象必须补齐全部服务。
- 如果某个信息化对象未来需要新增其他安全技术模块、服务或措施，必须由用户单独找该表架构师确认后再添加；Codex 不得自动判断并补齐。
- 原 `serviceModuleCoverageGapCandidate` / `moduleServiceCoverageGapCandidate` 口径废弃：`581 / 59` 只能作为全局目录对照项，不是业务缺口、不是错误、不是替换阻断项、不是业务待确认主流程。
- `1:N` 关系是允许的业务结构；审计只检查其能否被正确导出、展示和核对，不把同一服务关联多个模块 / 措施 / 系统本身视为错误。

Environment Mapping 后续 hard error / blocker 仅包括：

| 检查项 | 说明 |
|---|---|
| 服务引用一致性 | 已填写的安全技术服务必须存在于安全技术服务清单，且 code / title / id 与字典一致 |
| 模块 / 措施引用一致性 | 已填写的安全技术模块 / 措施必须存在于对应字典，且名称 / code / id 一致 |
| 服务 + 模块 / 措施组合一致性 | 已填写的服务和模块 / 措施组合必须符合安全技术模块清单或已确认措施来源关系 |
| 系统 + 模块归属一致性 | 已填写的安全系统和安全技术模块归属必须符合安全技术模块清单 |
| 信息化环境 / 信息化对象引用 | 已填写的信息化环境和信息化对象必须存在于架构师定义的环境对象结构 |
| 关系生成完整性 | `pendingRelationCount` 和 `invalidRowCount` 必须为 0 |
| 运行包字段完整性 | 正式导出包不得缺失页面展示真实业务关系所需字段 |

## 3.2.2 LC-DT 阶段 / 策略映射选择性引用规则

`LC-DT 数据生命周期` 和 `LC-DT 安全技术服务、模块、策略映射表` 中已填写的阶段 / 策略级安全技术服务、模块和措施，就是该阶段或策略的正式定义。

执行规则：

- 字典中的安全技术模块-安全技术服务集合用于校验模块自身的全局权威关系；不得反推每个 LC-DT 阶段或策略必须补齐该模块的全部安全技术服务。
- LC-DT 阶段 / 策略映射应以源表实际填写的安全技术服务为准；模块短名或别名可以归一到字典权威模块，但归一不等于扩展服务集合。
- 如果某个 LC-DT 阶段或策略未来需要新增其他安全技术服务、模块或措施，必须由用户单独确认源表业务口径后再添加；Codex 不得根据全局模块字典自动补齐。
- 审计 hard error 只检查已填写服务是否存在于安全技术服务清单、已填写模块 / 措施是否能归一到字典、两张 LC-DT 表之间是否存在真实服务集合不一致，以及 code / title / id 是否一致；选择性引用本身不是错误。

当前已确认示例：

- `加工/使用（US）/ US.07 脱敏` 实际引用 `I-NT&T-PD.DP-02 网络动态数据脱敏`、`I-AP&T-PD.DP-02 应用动态数据脱敏`、`I-DI&T-PD.DP-02 静态数据脱敏`。
- `提供（PR）/ PR.05 脱敏` 实际引用 `I-DI&T-PD.DP-02 静态数据脱敏`、`I-NT&T-PD.DP-02 网络动态数据脱敏`、`I-AP&T-PD.DP-02 应用动态数据脱敏`。
- `加工/使用（US）/ US.08 水印` 实际引用 `I-AP&T-PD.DP-01 应用页面水印`、`I-OS&T-PD.DP-01 操作系统屏幕水印`、`I-DI&T-PD.DP-01 数据内容水印`。
- `提供（PR）/ PR.07 水印` 实际引用 `I-AP&T-PD.DP-01 应用页面水印`、`I-OS&T-PD.DP-01 操作系统屏幕水印`、`I-DI&T-PD.DP-01 数据内容水印`。
