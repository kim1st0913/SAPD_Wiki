...
| TC-014 | 5173 服务 | 前端 / 后端 / 数据改动后 | 5173 只允许默认 `stable` Runtime；fixture、dev、ephemeral、自定义数据路径和显式测试 MCP Runtime 必须使用非 5173 端口，被拒绝的 restart 不得停止既有 stable 服务 | `python3 scripts/dev_server_guard.py --status` + `PYTHONPATH=src python3 -m unittest tests.mcp_integration.test_reserved_preview_port` |
