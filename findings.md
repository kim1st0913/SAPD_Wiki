...
- 2026-08-25 stable 5173 的健康合同必须覆盖当前已启用的 candidate-only 知识模块，不能只检查
  首页、通用 health 和旧 Batch 1 projection。安全运行候选通过显式 Runtime 配置绑定到持久候选
  目录；启动前校验候选 SHA，启动后校验 `/api/v1/security-operations` 返回同一 digest。隔离
  5189 仅用于实现验收，验收完成后关闭，不作为长期 Web 入口。
- 2026-08-24 安全运行产品结构冻结为“一级安全运行 + 七个二级页面 + 一个统一知识主轴 + 数据
  交叉视角 + 三层内容版本”：`总体思路 / 运行体系 / 威胁建模 / 调研与核验 / 运营指标 /
  成果与来源` 继续使用独立 `/security-operations/*` 路由，并新增独立 `完整正文` Tab，与“安全指南”
  并列。`保护对象牵引的
  安全运营总体思路` 作为主轴，数据安全六大模块融入保护对象、资产、漏洞、威胁和运营机制，
  不建立并列第二知识根。数据是广泛存在的关键价值载体，但不能替代业务连续性、服务可用性、
  身份信任、控制完整性和特定物理 / 工业安全等保护目标。
- 2026-08-24 七个二级页面纳入同一次 candidate-only Web 首发，`总体思路`
  同时作为主线总览，表达“为什么运行 → 如何运转 → 威胁是什么 → 如何调研核验 → 如何评价并
  改进”的闭环；`成果与来源` 是横向证据底座，不是假装成第六个线性步骤。技术身份、能力映射
  和图示不再形成用户逐项审核队列：有来源且关系合理的映射默认接纳为候选，证据不足由系统暂缓，
  图示原样 `source-only` 保留且不确认图意。业务人工复核仅是可选的产品内容 / UAT 反馈，不是
  Gate 阻断，也不得继续把 236 / 264 等旧审核流列为用户待办。
- 2026-08-24 页面职责进一步冻结：抽象归纳、文档关联与六阶段导航只在 `总体思路`；
  `完整正文` 只承载 `实战化安全运行解决方案（2025版 V0.2）`，去掉文档标题和源目录两个重复
  结构节点后以 104 个正文标题节点、28 张原图和按需目录抽屉阅读；`运行体系` 只承载 13 个运行
  活动、8 个运营维度、7 个数据与跨域视角及其相关能力映射，不复制全文。任何自编问题、摘要或
  要点必须显示 `整理稿 · 待确认`，来源事实与原图不加此标签。
- 2026-08-25 总体思路来源轴更正：旧“六阶段整理稿”退出主展示；`总体思路` 必须严格沿
  `01-000 保护对象牵引的安全运营总体思路` 第 1—10 章组织，第 4 章五个核心思路是互动核心，
  第 5—8 章是其展开。V0.2、调研方案、威胁资料和指标资料只在对应章节作为深化入口；`30-010`
  数据安全运营体系建设方案降为 V0.2 体系中的数据视角补充，不得形成并列体系或第二知识根。
  页面可增加导航性标签与操作提示，但不得把模型自行概括当作来源正文；若新增业务摘要，必须显式
  交给用户确认。
- 2026-08-24 安全运行模块定位明确为“架构师设计知识参考”，不是安全运营平台：产品价值是帮助
  架构师捋清方案主线、检查需要考虑的事项、获得方法 / 问题 / 规则 / 指标 / 图示和来源依据。
  运营指标只保存定义、公式、建议数据源、使用条件和来源，不接 SIEM / SOAR / CMDB / EDR /
  漏洞 / 工单等客户运行数据，不展示当前值、趋势、告警队列，也不执行处置或整改闭环。
- 2026-08-24 总体思路与现有安全能力框架的关系冻结为“引用既有稳定 ref，不复制第二棵能力树，
  不从标题或代码前缀猜关系”：当前 119 条映射分为 `formal_active=0 / policy=66 / deferred=53`。
  policy 候选可在 candidate Web 中作为候选参考展示，deferred 显式保留证据缺口；只有另行授权的
  formal freeze / apply 才能产生正式 active 对象并供正式 MCP 返回。candidate-only API、正式只读
  MCP 和用户库继续隔离。
- 2026-08-24 安全运行交付边界冻结为“八类来源资产不可变、完整语义 Web 主展示、结构化对象 /
  关系供受控 API 查询”：权威候选 bundle SHA-256 为
  `5fb801e33ef6bf907cd92179735c8ea34f47073a1f04b29fe73665fedeba68a6`。28 张图示仅保留来源位置和
  完整性状态，不提供图意确认、原件下载或外发；作者身份未知、版本缺口和许可限制必须如实展示，
  不能伪造为已确认。正式基础库、内容资产库、用户库和来源文件不得因候选 Web 集成而写入。
- 2026-08-24 非阻断漂移风险：安全运行前端个别标题仍包含冻结计数 `458 / 59 / 4`，与当前权威
  候选包一致；未来更换候选包时必须改由 manifest 驱动或增加计数漂移校验，不能让页面文案与
  API 投影悄然分叉。
