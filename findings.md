...
- 2026-07-23 本地 MCP 的工程权威设计为 `docs/01-architecture/sapd-wiki-local-mcp-requirements-and-prd-v0.5.md`，Work 已给出 `PASS` 且无新增技术 P0。Gate 0 由 `M0-T` 与 `D0` 两条授权独立的轨道组成：`M0-T` 只允许隔离 spike、synthetic fixture、测试证书、`ReadOnlyRuntimeProbe`、真实客户端和状态机/合同验证；`D0` 数据工作须单独授权，`D0-Pilot` 通过后才可评审 M1 计划，`D0-Release` 通过后真实摘要才可接入。正式 `ReadOnlyRuntimeContext` 属于 M1，生产双平台设置 UI 和窗口/可访问性验收属于 M3。TLS 优先比较加密 PKCS#8（PEM 编码）+ Keychain/DPAPI 随机口令，但口令不得经 CLI、环境变量或日志，系统信任变更须逐次授权与清理。公开摘要必须有唯一键、`summary_hash`、manifest digest 和内部可追责审核。T0 还必须固化 fixture RFC 8785 hash/ID 唯一性、D0-Pilot 非空门禁和 unsafe key-passphrase IPC `BLOCKED`。当前基础库仍无 `ai_summary`，15 个来源均非 public，正式可开放量为 0；stable identity 已覆盖 `4678/4678` 对象与 `7757/7757` 关系。执行计划已获后续用户授权并转入隔离 worktree；失败不得以 HTTP、长期 Token、用户库访问、放宽策略或把 fixture 当正式数据回退。
