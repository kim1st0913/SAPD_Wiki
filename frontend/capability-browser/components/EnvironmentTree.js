        ...
        (environment) => `
          <details class="environment-group" open>
            <summary class="environment-group-head">
              ...
              <span>${utils.escapeHtml(environment.objectCount ?? utils.list(environment.objects).length)}</span>
            </summary>
                ...
                .map(
                  (object) => `
                    ...
                    >
                      ...
                      <span>作用域 ${utils.escapeHtml(object.scopeCount ?? 0)} / 服务 ${utils.escapeHtml(object.serviceCount ?? 0)} / 模块 ${utils.escapeHtml(object.moduleCount ?? 0)}</span>
                    </button>
                  `,
                )
                .join("")}
