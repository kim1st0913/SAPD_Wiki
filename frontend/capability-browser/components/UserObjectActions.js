(function () {
  ...
  function render({ target, favorite, status = {}, noteOpen = false }) {
    ...
    const buttonLabel = isFavorite ? "取消收藏" : "收藏";
    const note = text(favorite?.note);
    const statusLabel = unavailable ? "用户库不可用" : isLoading ? "正在读取用户库" : isFavorite ? "已收藏" : "未收藏";
    const disabledAttr = isLoading || isSaving || unavailable ? " disabled" : "";
    const notePanel = noteOpen
      ? `
        <div class="user-action-note-panel">
          <textarea class="user-action-note-input" data-user-note-input="${escape(target.targetRef)}" rows="3" placeholder="记录这个对象的复核结论、待补充点或个人判断">${escape(note)}</textarea>
          <div class="user-action-note-tools">
            <span>${escape(target.objectLabel || "当前对象")}</span>
            <button class="user-action-note-save" type="button" data-user-note-save="${escape(target.targetRef)}"${disabledAttr}>保存备注并收藏</button>
          </div>
        </div>
      `
      : "";
    return `
      <section class="user-object-actions ${isFavorite ? "is-favorite" : ""} ${unavailable ? "is-unavailable" : ""}" data-user-target="${escape(target.targetRef)}" aria-label="用户对象操作">
        ...
        <div class="user-action-buttons">
          ...
          <button class="user-action-note-toggle" type="button" data-user-note-toggle="${escape(target.targetRef)}"${disabledAttr}>备注</button>
        </div>
        ${notePanel}
      </section>
    `;
