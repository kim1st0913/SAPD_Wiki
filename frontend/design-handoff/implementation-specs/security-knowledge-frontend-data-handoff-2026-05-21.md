...
短表统一规则：使用普通密集表格；多值字段以 chip 全量展示，不用 `+N` 省略；能在单元格内换行展示完整内容的，不再额外做归纳展开。只有长表且原始业务字段本身具备层级时，才保留分组展开。
...
| 应用系统目录 | `/knowledge/application-systems` | 无 |
...
| `application-systems` | 应用系统、定义、应用组件 |
...
- 数据来源为 `LC-AP 应用安全开发生命周期元素目录` 中的“应用系统类型目录”区域，而不是整张表所有区域。
- 主展示字段只保留原始业务字段：`应用系统`、`定义`、`应用组件`。
- 页面使用普通密集表格展示，每个 `应用系统` 一行，`应用组件` 全量展开为标签；不做折叠层级，也不使用 `+N` 截断；展示顺序优先使用原表来源行顺序。
- 同一 Sheet 中的 `软件开发类型目录` 可继续作为 LC-AP 参考数据保留，但不混入本页面。
- 不展示 `type`、`status`、`sources`、`row`、`column`、`raw_value`、`object_key` 等维护、来源或中间字段。
