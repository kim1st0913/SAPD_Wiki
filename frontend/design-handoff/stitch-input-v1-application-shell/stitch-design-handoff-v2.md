...
- 说明类内容不放在评估工作台主界面，统一归入 `安全指南 > 成熟度模型使用方法`。
- 能力数据不由 maturity 模块维护，统一引用 `知识库字典 > 安全能力清单` 的权威数据。
...
| SAPD 成熟度评估 | `domain-module` | 评估模板 / 填报会话 / 结果 | 评分填报、结果摘要、成熟度分析、报告入口 | 关系画布主界面、伪造未建设功能、方法说明页、独立能力主数据维护 | 后续 maturity 专用契约；能力数据引用 `知识库字典 > 安全能力清单` | 已纳入菜单规划，代码另开会话 | P2 |
