...
- 当前主控主线：先收敛执行线和 checkpoint，再治理 `OI-132 / EL-024` 数据加载稳定性；之后再进入 `OI-133 / EL-025` ArchiMate 建模语言页优化或 `OI-128` 最小用户写入入口。
- Open Issues 当前未关闭：`OI-038`、`OI-128`、`OI-132`、`OI-133`。其中 `OI-132` 已进入第一轮前端加载稳定性治理，暂不关闭，后续继续做安全能力映射四 tab 空态可信度专项回归。
...
- `OI-132`：安全能力映射页数据加载稳定性与空态可信度治理，状态 `治理中`。
