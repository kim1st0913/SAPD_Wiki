...
- 当前主控主线：`OI-128A/B` 用户动作入口已实现并跨页面复用；后续 UI 基准改为 `OI-128C` 右侧浮层批注抽屉与工作台，不再延续横向 `加入关注清单 / 收藏备注` 条。
...
- `OI-128`：USER-WRITE-UI-1：批注 / 工作台用户写入入口，状态 `部分完成`；`OI-128A/B` 已实现，下一步按 `OI-128C` 改为右侧浮层批注抽屉。
