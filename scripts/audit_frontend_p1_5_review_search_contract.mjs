...
async function main() {
  ...
  assert(includesAll(packagedServerSource, ["self.export_dir = self.resolve_export_dir(self.config.get(\"download_dir\"))", 'self.export_category_dir("issues")', '"output_path": str(output_path)', '"download_dir": str(self.export_dir)']) && includesAll(macosWrapperSource, ['title: "文件下载路径"', "changeDownloadPath", 'object["download_dir"] = settings.downloadDirectory.path']), "macOS App 未把用户选择的文件下载路径及 Issue 分类目录写入运行时配置", issues);
