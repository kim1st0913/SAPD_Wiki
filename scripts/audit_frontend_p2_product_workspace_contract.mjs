...
async function main() {
  ...
  assert(includesAll(apiSource, ["def dashboard_knowledge_summary", '"information_environments"', '"environment_segment_types"', '"information_objects"', '"environment_master_records"', '"application_system_types"', '"work_functions"', '"standard_frameworks"', '"lc_ap_stages"', '"lc_dt_stages"', '"slide_decks"']), "P2-1 服务端轻量统计字段不完整", issues);
  assert(serverSource.includes('/api/v1/dashboard/knowledge-summary'), "P2-1 5173 开发服务未暴露轻量统计接口", issues);
  assert(includesAll(workspaceSource, ["SYSTEM OVERVIEW", "系统数据总览", "信息化环境", "安全生命周期", "指南与知识表达", "幻灯片指南", "建模语言", "成熟度指南", "全局导航数据", "应用系统 / 组件", "catalog.application_components", "secondaryNavigationStats", "dashboard-stat-mosaic-secondary"]), "P2-1 系统统计或全局导航数据合并表达不完整", issues);
  assert(includesAll(workspaceSource, ["function renderInsightValue", "dashboard-insight-value is-static", "dashboard-insight-value is-link", 'route: "/development-security"', 'route: "/data-security"', 'route: "/guides/security-architecture-modeling-language"', 'route: "/guides/maturity-model-usage"', 'route: "/knowledge/environment-objects"', 'route: "/knowledge/application-systems"', 'route: "/knowledge/technical-modules"', 'route: "/knowledge/technical-measures"', 'route: "/knowledge/work-items"', 'route: "/knowledge/processes"']), "Dashboard 分类统计未按独立业务页面分配精确路由", issues);
  ...
  assert(includesAll(appShellSource, ['eyebrow: "SAPD WIKI"', 'title: "全局导航"', "查看系统数据、处理待办工作，并按主要业务流程进入对应页面。", "hideTypeLabel: true"]), "P2-1 首页 AppShell 标题内容未统一为全局导航", issues);
  assert(!workspaceSource.includes("工作台与知识库概览") && !workspaceSource.includes("dashboard-p2-heading"), "P2-1 首页正文仍重复持有页面标题", issues);
  assert(includesAll(workspaceSource, ["安全能力", "能力关注点", "安全技术服务", "安全技术模块 / 措施", "标准控制项", "环境主数据"]), "P2-1 基础统计对象不完整", issues);
  ...
  assert(config.p2_1?.recentWork?.issueLimit === 3 && config.p2_1?.recentWork?.maturityLimit === 3, "P2-1 待办展示数量契约不是 ISSUE 3 条 / 成熟度 3 个", issues);
  assert(includesAll(workspaceSource, ["recentIssues.slice(0, 3)", "projects.slice(0, 3)", "dashboard-overflow-menu", "ISSUE清单更多", "成熟度评估更多", "查看全部", "data-dashboard-issue-id"]), "P2-1 待办工作列表或更多菜单实现不完整", issues);
  assert(config.p2_1?.dashboardComposition?.rule === "action-lists-are-bounded-subsections-not-recent-content-walls" && config.p2_1?.dashboardComposition?.forbiddenRegression === "flat-list-only-workbench", "P2-1 待办中心与内部任务列表的层级契约未固化", issues);
  assert(config.p2_1?.dashboardComposition?.title === "待办工作" && config.p2_1?.dashboardComposition?.crossStreamSummary === "forbidden" && !config.p2_1?.dashboardComposition?.layers?.includes("work-status-overview"), "P2-1 ISSUE 与成熟度评估仍可能被混进统计区", issues);
  assert(config.p2_1?.usageGuide?.pattern === "single-document-directory" && config.p2_1?.usageGuide?.groups?.operational?.length === 3 && !config.p2_1?.usageGuide?.groups?.knowledge && config.p2_1?.usageGuide?.routes?.length === 3 && config.p2_1?.usageGuide?.placement?.primary === "beside-work-items" && config.p2_1?.usageGuide?.columnBalance === "work-items-65-quick-start-35", "P2-1 快速开始与待办并列布局契约不完整", issues);
  assert(includesAll(workspaceSource, ["function renderUsageGuide", "function renderUsageGuideRow", "QUICK START", "快速开始", "MCP 集成", "ISSUE 生成与管理", "成熟度评估", "生成本机安全连接证书", "业务页面创建 Issue", "创建项目 → 选择模板", "进入设置", "进入清单", "开始评估", "状态、优先级、页面定位、处理结果和导出统一管理。", "项目、模板、评分、复核、结果与报告统一管理。", 'route: "/settings/ai-integration"', 'route: "/workbench/annotations"', 'route: "/workbench/maturity"', "dashboard-usage-guide-item"]), "P2-1 快速开始、证书步骤、工作流摘要或操作不完整", issues);
  assert(excludesAll(workspaceSource, ["知识库概况", "知识库与指南说明", "KNOWLEDGE GUIDE", "dashboard-dictionary-summary"]), "P2-1 首页仍保留知识概况或独立导航统计表", issues);
  assert(!workspaceSource.includes("选择知识域 → 展开目录") && !workspaceSource.includes("打开安全指南 → 选择主题"), "P2-1 知识库与指南静态说明仍保留重复流程解释", issues);
  assert(workspaceSource.indexOf("${renderKnowledge(summary)}") < workspaceSource.indexOf('<div class="dashboard-p2-action-grid">') && workspaceSource.indexOf("${renderWorkbench({ issueSummary, recentIssues, issueDataState, maturitySummary })}") < workspaceSource.indexOf('${renderUsageGuide(guides.operational, "operational")}'), "P2-1 首页顺序不是统计、待办、快速开始", issues);
  assert(includesAll(cssSource, [".dashboard-p2-action-grid", "grid-template-columns: minmax(620px, 1.45fr) minmax(360px, 0.8fr)", ".dashboard-usage-guide", ".dashboard-usage-guide-list", ".dashboard-usage-guide-copy", ".dashboard-usage-guide-action", ".dashboard-usage-guide-item", ".dashboard-stat-mosaic-secondary", ".dashboard-navigation-stats", ".dashboard-navigation-stat:first-child", "grid-auto-rows: minmax(52px, 1fr)", "align-items: stretch", "font-size: 13px", "font-size: 11px"]), "P2-1 首页不规则统计、65/35 行动区或待办填充布局不完整", issues);
  assert(config.p2_1?.knowledgeLayout?.pattern === "asymmetric-integrated-color-mosaic" && config.p2_1?.knowledgeLayout?.globalNavigationData === "merged-into-statistics-mosaic" && config.p2_1?.knowledgeLayout?.crossGrainAggregation === "forbidden", "P2-1 不规则统计与导航数据合并契约未固化", issues);
  assert(includesAll(workspaceSource, ["待办工作", "renderWorkstreamLead", "dashboard-workstream-lead", "dashboard-workstream-list-head", "dashboard-workbench-foot", "处理架构评审问题", "继续客户评估项目", "继续处理", "继续评估"]), "P2-1 待办工作仍缺少独立工作流、快捷继续或分组摘要", issues);
  ...
  assert(includesAll(cssSource, ["width: min(100%, 1560px)", "dashboard-stat-mosaic-secondary", "dashboard-navigation-stats", "dashboard-workstream-lead", "dashboard-workstream-list-head", "dashboard-recent-row", "dashboard-overflow-popover", "dashboard-workbench-foot", "grid-template-columns: repeat(6, minmax(0, 1fr))", "grid-auto-rows: 64px", "tone-ocean", "tone-sunset", "tone-orchid", "oklch"]), "P2-1 Apple 待办工作、紧凑单排核心统计或次级统计样式不完整", issues);
  assert(!/\.dashboard-knowledge-stat\[data-dashboard-stat=[^}]+grid-(?:column|row):\s*span/s.test(cssSource), "P2-1 核心统计仍存在跨列或跨行布局，无法保持宽屏单排", issues);
  ...
  assert(includesAll(indexSource, ["p2-product-workspace.css?v=p2-product-workspace-20260727-1-compact-overview", "P2ProductWorkspace.js?v=p2-product-workspace-20260726-7-action-dashboard", "maturity-report-v2-authoring-20260718-1", "p2-workbench-dashboard-20260716-1", "p2-dashboard-header-20260716-1", "p2-default-collapse-20260716-1", "p2-dashboard-summary-20260716-1", "p2-2-directory-navigation-20260716-3"]), "P2 V9 入口或缓存版本不完整", issues);
  ...
  if (baseUrl) {
    try {
      const [liveIndex, liveApp, liveAppShell, liveDirectory, liveWorkspace, liveMaturity, liveCss, liveGuide, liveSummary] = await Promise.all([
        ...
        fetchText(baseUrl, "components/P2ProductWorkspace.js?v=global-navigation-data-20260726-1"),
        fetchText(baseUrl, "components/MaturityAssessmentWorkbench.js?v=maturity-report-v2-authoring-20260718-1"),
        fetchText(baseUrl, "p2-product-workspace.css?v=p2-product-workspace-20260726-7-action-dashboard"),
      ...
      assert(liveIndex.includes("p2-product-workspace-20260727-1-compact-overview") && liveIndex.includes("p2-product-workspace-20260726-7-action-dashboard") && liveIndex.includes("p2-workbench-dashboard-20260716-1") && liveIndex.includes("p2-dashboard-header-20260716-1") && liveIndex.includes("p2-2-directory-navigation-20260716-3"), "5173 未加载 P2 V9 入口", issues);
      ...
      assert(liveAppShell.includes('title: "全局导航"') && liveAppShell.includes("查看系统数据、处理待办工作，并按主要业务流程进入对应页面。") && liveAppShell.includes("hideTypeLabel: true"), "5173 AppShell 未加载全局导航标题契约", issues);
      ...
      assert(liveWorkspace.includes("recentIssues.slice(0, 3)") && liveWorkspace.includes("projects.slice(0, 3)") && liveWorkspace.includes("待办工作") && !liveWorkspace.includes("dashboard-workbench-overview") && liveWorkspace.includes("dashboard-workstream-lead") && liveWorkspace.includes("dashboard-overflow-menu") && liveWorkspace.includes("SYSTEM OVERVIEW") && liveWorkspace.includes("全局导航数据") && liveWorkspace.includes("快速开始") && !liveWorkspace.includes("知识库与指南说明") && liveWorkspace.includes("生成本机安全连接证书") && liveWorkspace.includes("dashboard-stat-mosaic-secondary"), "5173 首页缺少系统统计、待办工作、快速开始或证书步骤", issues);
      ...
      assert(!liveCss.includes("dashboard-workbench-overview") && liveCss.includes("dashboard-workstream-lead") && liveCss.includes("dashboard-overflow-popover") && liveCss.includes("dashboard-workbench-foot") && liveCss.includes("dashboard-stat-mosaic-secondary") && liveCss.includes("dashboard-navigation-stats") && liveCss.includes("dashboard-p2-action-grid") && liveCss.includes("grid-template-columns: repeat(6, minmax(0, 1fr))") && liveCss.includes("grid-auto-rows: 64px") && liveCss.includes("grid-template-rows: auto minmax(0, 1fr) auto") && liveCss.includes("justify-content: flex-end"), "5173 CSS 不是 P2 V9 或缺少紧凑统计 / 行动区样式", issues);
      ...
      assert(summary?.environment?.information_environments === 10 && summary?.environment?.environment_segment_types === 16 && summary?.environment?.information_objects === 51 && summary?.environment?.scope_types === 6, "5173 环境 / 子类 / 对象 / 作用域统计不符合权威投影", issues);
      assert(summary?.catalog?.environment_master_records === 77 && summary?.catalog?.application_system_types === 3 && summary?.catalog?.application_components === 13 && summary?.catalog?.work_functions === 86 && summary?.catalog?.standard_frameworks === 7, "5173 全局导航目录统计未覆盖环境主数据、应用系统、职能或标准", issues);
      assert(summary?.lifecycles?.lc_ap_stages === 8 && summary?.lifecycles?.lc_dt_stages === 7, "5173 生命周期阶段统计不符合权威投影", issues);
      assert(summary?.content?.html_documents === 3 && summary?.content?.slide_decks === 3 && summary?.content?.diagram_views === 1 && summary?.content?.guide_pages === 2, "5173 指南与内容统计不符合权威投影", issues);
    } catch (error) {
