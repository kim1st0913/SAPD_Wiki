...
async function main() {
  ...
  assert(includesAll(workspaceSource, ["KNOWLEDGE OBSERVATORY", "信息化环境", "安全生命周期", "指南与知识表达", "幻灯片指南", "建模语言", "成熟度指南", "全局导航数据", "应用系统 / 组件", "catalog.application_components"]), "P2-1 新增统计表达不完整", issues);
  ...
  assert(config.p2_1?.usageGuide?.pattern === "single-document-directory" && config.p2_1?.usageGuide?.groups?.operational?.length === 3 && config.p2_1?.usageGuide?.groups?.knowledge?.length === 2 && config.p2_1?.usageGuide?.routes?.length === 3 && config.p2_1?.usageGuide?.placement?.left === "above-work-items" && config.p2_1?.usageGuide?.columnBalance === "equal-height-column-wrappers-with-last-panel-fill", "P2-1 首页分组说明、上下顺序或等高布局契约不完整", issues);
  assert(includesAll(workspaceSource, ["function renderUsageGuide", "function renderUsageGuideRow", "业务流程使用说明", "知识库与指南说明", "MCP 集成", "知识库概况", "指南内容与阅读", "ISSUE 生成与管理", "成熟度评估", "生成本机安全连接证书", "业务页面创建 Issue", "创建项目 → 选择模板", "进入设置", "进入清单", "开始评估", 'route: "/settings/ai-integration"', 'route: "/workbench/annotations"', 'route: "/workbench/maturity"', "dashboard-usage-guide-item", "dashboard-p2-column-operational", "dashboard-p2-column-knowledge"]), "P2-1 首页说明分组、证书步骤、业务流、操作或左右列布局不完整", issues);
  assert(!workspaceSource.includes("选择知识域 → 展开目录") && !workspaceSource.includes("打开安全指南 → 选择主题"), "P2-1 知识库与指南静态说明仍保留重复流程解释", issues);
  assert(workspaceSource.indexOf('${renderUsageGuide(guides.operational, "operational")}') < workspaceSource.indexOf("${renderWorkbench({ issueSummary, recentIssues, issueDataState, maturitySummary })}"), "P2-1 业务流程说明未置于工作事项上方", issues);
  assert(includesAll(cssSource, [".dashboard-usage-guide", ".dashboard-usage-guide-list", ".dashboard-usage-guide-copy", ".dashboard-usage-guide-action", ".dashboard-usage-guide-item", ".dashboard-p2-column", "align-items: stretch", "flex: 1 1 auto", "grid-auto-rows: minmax(88px, 1fr)", "font-size: 13px", "font-size: 11px"]), "P2-1 首页说明字号、静态知识说明或等高列实现不完整", issues);
  ...
  assert(includesAll(indexSource, ["p2-product-workspace.css?v=p2-product-workspace-20260717-8", "P2ProductWorkspace.js?v=p2-product-workspace-20260717-6-work-items-mosaic", "global-navigation-data-20260726-1", "maturity-report-v2-authoring-20260718-1", "p2-workbench-dashboard-20260716-1", "p2-dashboard-header-20260716-1", "p2-default-collapse-20260716-1", "p2-dashboard-summary-20260716-1", "p2-2-directory-navigation-20260716-3"]), "P2 V8 入口或缓存版本不完整", issues);
  ...
  if (baseUrl) {
    try {
      ...
      assert(liveIndex.includes("p2-product-workspace-20260717-8") && liveIndex.includes("p2-product-workspace-20260717-6-work-items-mosaic") && liveIndex.includes("global-navigation-data-20260726-1") && liveIndex.includes("p2-workbench-dashboard-20260716-1") && liveIndex.includes("p2-dashboard-header-20260716-1") && liveIndex.includes("p2-2-directory-navigation-20260716-3"), "5173 未加载 P2 V8 入口", issues);
      ...
      assert(!liveCss.includes("dashboard-workbench-overview") && liveCss.includes("dashboard-workstream-lead") && liveCss.includes("dashboard-overflow-popover") && liveCss.includes("dashboard-workbench-foot") && liveCss.includes("dashboard-insight-deck") && liveCss.includes("dashboard-usage-guide-list") && liveCss.includes("grid-template-columns: repeat(6, minmax(0, 1fr))") && liveCss.includes("data-dashboard-stat=\"capabilities\"") && liveCss.includes("grid-template-rows: auto minmax(0, 1fr) auto") && liveCss.includes("justify-content: flex-end"), "5173 CSS 不是 P2 V8 或缺少使用说明样式", issues);
