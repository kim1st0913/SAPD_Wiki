...
def main() -> int:
    ...
    require(all(score_workbook["评估模板"].cell(1, column).value in {None, expected} for column, expected in ((9, "组织与角色"), (10, "制度与流程"), (11, "平台与工具"), (12, "数据与信息"), (13, "目标等级"))), "assessment XLSX must expose four score dimensions and target level")
    ...
    optional_note_exchange = mutate_exchange_workbook(score_export["package"], lambda workbook: setattr(workbook["评估模板"]["N3"], "value", ""))
    ...
    conflicting_exchange = mutate_exchange_workbook(score_export["package"], lambda workbook: setattr(workbook["评估模板"]["M3"], "value", "L2"))
    ...
    require(report_model["schemaVersion"] == "sapd-maturity-report-model-v2" and set(report_sections) == required_report_sections and report_model["resultVersion"]["resultHash"] == completed_result["calculationRun"]["resultHash"], "report snapshot must expose one versioned, extensible model tied to the exact calculation result")
