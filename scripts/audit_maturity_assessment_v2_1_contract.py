...
def main() -> int:
    ...
    require(all(token in report["markdown"] for token in ("## 成熟度雷达", "## 完整 L2 能力结果", "## 总体 L2 能力 Top 10", "## 四维 L2 能力 Top 10", "## 完整评分明细附录", "管理层摘要审计", "安全架构组")) and all(token in report["html"] for token in ("contenteditable='true'", "<svg class='radar-svg'", "全 L2 能力分组雷达", "总体四维成熟度雷达", "完整 L2 能力结果", "完整评分明细附录", "管理建议审计", "管理层批准预算")) and report["improvementRoadmap"][0]["capabilityId"] == roadmap_gap["capabilityId"], "Markdown and self-contained HTML must consume the same complete model, including editable narratives, both radars, rankings, roadmap and appendix")
    ...
    require(all(token in report_tab_block for token in ("评估概况", "关键发现", "提升建议", "下一步计划", "maturity-v37-report-shell", "renderReportOverall(detail)", "renderReportCategoryCoverage(detail)", "renderReportPriorityTable(detail)", "renderReportResultAppendix(detail)", "data-maturity-report-field", "data-maturity-capability-radar", "data-maturity-result-radar", "能力类别评分")) and component.count('data-maturity-action="download-report"') == 2 and all(token in component for token in ('data-format="markdown"', 'data-format="html"', 'if (format === "markdown") downloadBlob', 'if (format === "html") downloadBlob')) and all(token in styles for token in (".maturity-v37-report-shell", ".maturity-v37-report-nav", ".maturity-v37-report-dashboard", ".maturity-v37-report-narrative", ".maturity-v37-report-appendix")) and 'data-format="json"' not in component and 'data-format="package"' not in component and all(token in report_config for token in ("markdown:", "html:", "sapd-maturity-report-model-v2", "executiveSummary", "keyFindings", "managementRecommendations", "nextSteps", "radar_suite", "score_appendix")) and "json:" not in report_config, "assessment report UI and configuration must expose the selected executive workspace, editable narratives, real result radars, full appendices and two independent Markdown/HTML downloads")
