...
def read_exchange_workbook(package: dict):
    return load_workbook(BytesIO(base64.b64decode(package["workbookBase64"])), data_only=False)


def mutate_exchange_workbook(package: dict, mutation) -> dict:
    workbook = read_exchange_workbook(package)
    mutation(workbook)
    output = BytesIO()
...
def main() -> int:
    ...
    delivery_workspace = build_maturity_workspace(workbench, project_profile="delivery")
    ...
    require(workspace.get("projectProfile") == "development" and set(workspace["projectDetails"]) == {f"demo-project-{index:03d}" for index in range(1, 6)}, "the development workspace must expose five controlled test projects")
    require(delivery_workspace.get("projectProfile") == "delivery" and set(delivery_workspace["projectDetails"]) == {"demo-project-001", "demo-project-002"}, "the delivery workspace must package exactly one in-progress and one completed test case")
    group_demo = workspace["projectDetails"]["demo-project-001"]
    completed_demo = workspace["projectDetails"]["demo-project-002"]
    require(
        group_demo["project"].get("controlledDemoRevision") == "two-test-cases-in-progress-20260720"
        and group_demo["project"].get("status") == "scoring"
        and group_demo["result"]["summary"].get("statisticsReady") is False
        and group_demo["result"]["summary"].get("targetBelowCurrentCount") == 3,
        "the in-progress test case must remain scoring and must not expose formal statistics",
    )
    require(
        completed_demo["project"].get("controlledDemoRevision") == "two-test-cases-completed-20260720"
        and completed_demo["project"].get("status") == "completed"
        and completed_demo["result"]["summary"].get("statisticsReady") is True
        and completed_demo["result"]["summary"].get("completionRate") == 100,
        "the completed test case must be formally complete",
    )
    ...
    require(result["calculationRun"]["algorithmVersion"] == "sapd-maturity-v2.2.0", "calculation run must freeze the per-dimension target algorithm version")
    require(result["scoreItemResults"][0]["targetDimensionResults"] == {key: 4.0 for key in DIMENSIONS}, "legacy target level must expand to all four target dimensions")
    equal_target = scored_entry("score-1", levels=("L1", "L2", "L3", "L4"), target="L4")
    equal_target["targetElements"] = copy.deepcopy(equal_target["elements"])
    equal_target_result = calculate_maturity_assessment({"project": project, "template": template, "scoreEntries": [equal_target]})
    require(
        equal_target_result["scoreItemResults"][0]["targetBelowCurrent"] is False
        and equal_target_result["scoreItemResults"][0]["isComplete"] is True
        and equal_target_result["summary"]["statisticsReady"] is True,
        "target dimensions equal to current dimensions must remain valid and complete",
    )
    varied_target = copy.deepcopy(entry)
    varied_target["targetElements"] = {"organization": "L3", "process": "L4", "tool": "L4", "data": "L5"}
    varied_target["targetDimensionNotes"] = {key: f"{key} 目标说明" for key in DIMENSIONS}
    varied_result = calculate_maturity_assessment({"project": project, "template": template, "scoreEntries": [varied_target]})
    require(varied_result["summary"]["targetIndex"] == 4.0 and varied_result["summary"]["targetDimensionResults"] == {"organization": 3.0, "process": 4.0, "tool": 4.0, "data": 5.0}, "four target dimensions must aggregate with the configured weights")
    incomplete_target = copy.deepcopy(varied_target)
    incomplete_target["targetElements"].pop("data")
    incomplete_result = calculate_maturity_assessment({"project": project, "template": template, "scoreEntries": [incomplete_target]})
    require(incomplete_result["summary"]["completionRate"] == 0 and incomplete_result["scoreItemResults"][0]["targetIndex"] is None, "an explicitly incomplete targetElements object must not fall back to legacy targetLevel")
    ...
    dimension_conflict_entry = scored_entry("score-1", levels=("L4", "L1", "L1", "L1"), target="L3")
    dimension_conflict_entry["targetElements"] = {"organization": "L3", "process": "L3", "tool": "L1", "data": "L1"}
    dimension_conflict_result = calculate_maturity_assessment({"project": project, "template": template, "scoreEntries": [dimension_conflict_entry]})
    require(
        dimension_conflict_result["scoreItemResults"][0]["targetIndex"] >= dimension_conflict_result["scoreItemResults"][0]["currentIndex"]
        and dimension_conflict_result["scoreItemResults"][0]["targetBelowCurrent"] is True
        and dimension_conflict_result["scoreItemResults"][0]["targetDimensionConflicts"][0]["dimension"] == "organization",
        "a same-dimension target regression must block the item even when the weighted target is not below the weighted current result",
    )
    require(
    ...
    require(score_export["ok"] and score_export["package"]["schemaVersion"] == "maturity-score-exchange-v2.2", "score exchange export must identify the four-dimension target schema")
    score_workbook = read_exchange_workbook(score_export["package"])
    require(score_workbook.sheetnames == ["模板基础信息", "评估模板"], "business XLSX must contain exactly the basic-information and assessment sheets")
    visible_values = " ".join(str(cell.value or "") for sheet in score_workbook.worksheets for row in sheet.iter_rows() for cell in row)
    require(all(token not in visible_values for token in ("模板 ID", "快照 ID", "结构哈希", "客户名称", project["id"])), "business XLSX must not expose developer fields or customer identity")
    require(score_workbook["评估模板"]["I1"].value == "当前状态" and score_workbook["评估模板"]["Q1"].value == "目标状态", "assessment XLSX must expose paired current and target sections")
    require(all(score_workbook["评估模板"].cell(2, column).value == expected for column, expected in ((9, "组织与角色"), (10, "制度与流程"), (11, "平台与工具"), (12, "数据与信息"), (17, "组织与角色"), (18, "制度与流程"), (19, "平台与工具"), (20, "数据与信息"))), "assessment XLSX must expose four current and four target dimensions")
    ...
    optional_note_exchange = mutate_exchange_workbook(score_export["package"], lambda workbook: setattr(workbook["评估模板"]["U3"], "value", ""))
    ...
    def mark_not_applicable(workbook):
        sheet = workbook["评估模板"]
        sheet["H3"] = "不适用"
    optional_na_exchange = mutate_exchange_workbook(score_export["package"], mark_not_applicable)
    ...
    require(optional_na_import["ok"] and optional_na_import["batch"]["status"] == "success" and optional_na_import["scoreEntries"][0]["status"] == "not_applicable" and optional_na_import["scoreEntries"][0]["elements"] == {} and optional_na_import["scoreEntries"][0]["targetLevel"] is None, "score exchange import must ignore residual scores on a not-applicable row without creating blockers")
    def lower_all_targets(workbook):
        for column in ("Q", "R", "S", "T"):
            workbook["评估模板"][f"{column}3"] = "L2"
    conflicting_exchange = mutate_exchange_workbook(score_export["package"], lower_all_targets)
    ...
    changed_structure = mutate_exchange_workbook(score_export["package"], lambda workbook: setattr(workbook["评估模板"]["E3"], "value", "非法修改结构"))
    ...
    template_workbook = read_exchange_workbook(template_export["package"])
    require(all(template_workbook["评估模板"].cell(3, column).value in {None, ""} for column in range(8, 15)), "template export must leave applicability and every scoring field empty")
    template_import = import_maturity_template_exchange({"exchange": template_export["package"]})
    require(template_import["ok"] and template_import["template"]["id"] != template["id"] and template_import["template"]["type"] == "custom" and template_import["sourceTemplateType"] == "custom", "valid custom business workbook must import as a custom template")
    scored_template_exchange = mutate_exchange_workbook(template_export["package"], lambda workbook: setattr(workbook["评估模板"]["I3"], "value", "L3"))
    scored_template_import = import_maturity_template_exchange({"exchange": scored_template_exchange})
    require(not scored_template_import["ok"] and any(error.get("code") == "template_contains_scores" for error in scored_template_import["rowErrors"]), "template import must reject any workbook containing score data")
    ...
    require(base_template_export["ok"] and not base_template_import["ok"] and any(error.get("code") == "standard_template_import_forbidden" for error in base_template_import["rowErrors"]), "standard template workbooks must not be accepted as custom template imports")
    ...
    completed_result = calculate_maturity_assessment(completed)
    roadmap_gap = completed_result["gapItems"][0]
    report = create_maturity_report_snapshot({"project": completed["project"], "template": completed["template"], "scoreEntries": completed["scoreEntries"], "narrative": {"executiveSummary": "管理层摘要审计", "keyFindings": "关键发现审计", "managementRecommendations": "管理建议审计", "nextSteps": "下一步审计"}, "improvementRoadmap": [{"capabilityId": roadmap_gap["capabilityId"], "action": "完成高优先级能力整改并复核", "owner": "安全架构组", "resources": "2 人月", "dependencies": "管理层批准预算", "status": "已确认"}]})
    incomplete_report = create_maturity_report_snapshot({"project": group_demo["project"], "template": group_demo["template"], "scoreEntries": group_demo["scoreEntries"]})
    require(incomplete_report.get("ok") is False and incomplete_report.get("dataState") == "assessment_incomplete" and incomplete_report.get("error") == "assessment_must_be_completed_before_report_generation", "in-progress assessment must be rejected before any report artifact is generated")
    ...
    report_sections = {item["id"]: item for item in report_model["sections"]}
    ...
    ordered_l0_ids = [
        item["id"]
        for item in sorted(
            [item for item in completed["template"]["categories"] if item.get("capabilityLevel") == "L0"],
            key=lambda item: (item.get("sortOrder") if isinstance(item.get("sortOrder"), (int, float)) else float("inf"), item.get("code") or item.get("name") or ""),
        )
    ]
    expected_radar_axis_ids = [
        row["id"]
    ...
    require(actual_radar_axis_ids == expected_radar_axis_ids, "HTML report and report-page radar must share the template sortOrder axis sequence")
    require(len(report_sections["capability_results"]["data"]) == len(completed_result["capabilityResults"]) and len(report_sections["score_appendix"]["data"]) == len(completed_result["scoreItemResults"]) and len(report_sections["overall_rankings"]["data"]["leading"]) <= 10 and len(report_sections["overall_rankings"]["data"]["improvement"]) <= 10 and all(len(item["leading"]) <= 10 and len(item["improvement"]) <= 10 for item in report_sections["dimension_rankings"]["data"]), "report model must retain complete L2 and score-item tables together with bounded overall and per-dimension Top 10 rankings")
    require({item["key"]: item["label"] for item in report_model["narrativeFields"]} == {"executiveSummary": "评估概况", "keyFindings": "关键发现", "managementRecommendations": "提升建议", "nextSteps": "下一步计划"}, "report narrative fields must preserve API keys while exposing the approved report labels")
    require(all(token in report["markdown"] for token in ("## 成熟度雷达", "## 完整 L2 能力结果", "## 总体 L2 能力 Top 10", "## 四维 L2 能力 Top 10", "## 完整评分明细附录", "管理层摘要审计", "安全架构组")) and all(token in report["html"] for token in ("contenteditable='true'", "<svg class='radar-svg'", "radar-series-current", "radar-series-target", ">体系架构</text>", ">意识培养</text>", "全 L2 能力分组雷达", "总体四维成熟度雷达", "完整 L2 能力结果", "完整评分明细附录", "管理建议审计", "管理层批准预算")) and report["improvementRoadmap"][0]["capabilityId"] == roadmap_gap["capabilityId"], "Markdown and self-contained HTML must consume the same complete model, including visible partial radar series, shared compact labels, editable narratives, both radars, rankings, roadmap and appendix")
    require(all(token in report["html"] for token in ("executive-brief", "maturity-path", "maturity-track__progress", "target-readiness", "decision-focus", "executive-facts", "scorecard-main", "总体四维成熟度雷达", "APPENDIX · PAGE THREE", "第 3 页 · 附录", "评估口径说明", 'field.setAttribute("role", "textbox")')) and "结果哈希" not in report["html"] and "结果哈希" not in report["markdown"] and "报告快照：" not in report["html"] and "报告快照：" not in report["markdown"], "formal report outputs must use the approved management-first executive HTML template with dynamic maturity trajectory, retain the approved appendix and four-dimension radar, and exclude non-business snapshot/hash fields")
    changed_narrative_report = create_maturity_report_snapshot({"project": completed["project"], "template": completed["template"], "scoreEntries": completed["scoreEntries"], "narrative": {"executiveSummary": "另一个评估概况"}})
    changed_result_report = create_maturity_report_snapshot({"project": completed["project"], "template": completed["template"], "scoreEntries": [{**completed["scoreEntries"][0], "elements": dict.fromkeys(DIMENSIONS, "L5")}, *completed["scoreEntries"][1:]], "narrative": {"executiveSummary": "管理层摘要审计"}})
    require(changed_narrative_report["id"] != report["id"] and changed_result_report["id"] != report["id"], "snapshot hash must include the full report model, authored narrative and calculation-result version")
    original_user_db_path = maturity_api.USER_DB_PATH
    try:
        with tempfile.TemporaryDirectory(prefix="sapd-maturity-report-history-") as temporary_root:
            temporary_user_db = Path(temporary_root) / "Runtime" / "data" / "user" / "sapd_wiki_user.sqlite3"
            maturity_api.configure_runtime_paths(user_db=temporary_user_db)
            report_payload = {"project": completed["project"], "operation": "create"}
            first_artifact = maturity_api.persist_maturity_report_artifact(copy.deepcopy(report), report_payload)
            report_payload["operation"] = "update"
            second_artifact = maturity_api.persist_maturity_report_artifact(copy.deepcopy(report), report_payload)
            restored_artifact = maturity_api.load_maturity_report_artifact(
                project_id=completed["project"]["id"],
                artifact_id=first_artifact["persistence"]["artifactId"],
            )
            manifest = json.loads(
                (maturity_api._maturity_project_root(completed["project"]["id"]) / "manifest.json").read_text(encoding="utf-8")
            )
            require(
                first_artifact["persistence"]["artifactId"] != second_artifact["persistence"]["artifactId"]
                and len(manifest["artifacts"]) == 2
                and restored_artifact["html"] == report["html"]
                and restored_artifact["markdown"] == report["markdown"],
                "generated and updated reports must persist as separate recoverable artifacts beside the configured user database",
            )
    finally:
        maturity_api.configure_runtime_paths(user_db=original_user_db_path)
    ...
    directory_shell = (ROOT / "frontend" / "capability-browser" / "shared-directory-shell.css").read_text(encoding="utf-8")
    ...
    api_server_source = (ROOT / "src" / "sapd_wiki" / "api_server.py").read_text(encoding="utf-8")
    bundle_server_source = (ROOT / "scripts" / "run_local_server.py").read_text(encoding="utf-8")
    ...
    assessment_details_block = inspector_block.split('class="maturity-v15-assessment-details maturity-v33-target-validation"', 1)[1].split("</section>", 1)[0]
    ...
    project_search_data_block = component.split("function projectObjectSearchResults", 1)[1].split("function projectObjectSearchState", 1)[0]
    feedback_block = component.split("function renderFeedback", 1)[1].split("function showToast", 1)[0]
    results_tab_block = component.split("function renderResultsTab", 1)[1].split("function renderInternalAssessmentDetails", 1)[0]
    internal_details_block = component.split("function renderInternalAssessmentDetails", 1)[1].split("function renderGapTable", 1)[0]
    ...
    display_template_name_block = component.split("function displayTemplateName(detail)", 1)[1].split("function standardProjectTemplateName", 1)[0]
    ...
    require('model.activeTab === "scoring" ? ""' not in component and "maturity-v5-project-context" not in component and "maturityShellHeaderActions" in app_shell and all(token in component for token in ("syncMaturityShellHeader", "pageTitle.textContent = detail?.project?.templateWorkspace ? displayTemplateName(detail) : detail ? detail.project.name", "最近更新")) and 'model.activeTab === "report" ? " 评估报告" : ""' not in component, "all concrete maturity project tabs must keep one shared project identity and header geometry without report-only title or description variants")
    ...
    require("maturity-v4-scoring-shell" in component and ".maturity-v4-scoring-shell" in styles and "grid-template-columns: var(--maturity-score-directory-width) 6px minmax(0, 1fr)" in styles, "scoring must use a resizable left directory and right workbench layout")
    require(all(token in component for token in ("scoreDirectoryUiByProject", 'data-maturity-action="toggle-score-directory"', "data-maturity-score-directory-resizer", "beginScoreDirectoryResize", "aria-valuemin", "components.AppShell?.directoryPaneMetrics", "shell-directory-pane shell-directory-pane-has-meta", "shell-directory-resizer")) and all(token in styles for token in (".maturity-v4-scoring-shell.is-directory-collapsed", ".maturity-v4-scoring-shell.is-directory-collapsed .maturity-v4-score-workbench", ".maturity-v4-directory-resizer", ".maturity-v4-directory-expand-tab")) and all(token in app_shell for token in ("defaultWidth: 304", "minWidth: 240", "maxWidth: 520", "directoryPaneMetrics")) and all(token in directory_shell for token in ("shell-directory-head", "shell-directory-title", "shell-directory-action", "shell-directory-tree", "shell-directory-resizer")), "scoring directory must reuse the shared directory frame and 304 / 240—520 width contract while keeping full-pane collapse and resize behavior")
    ...
    require("maturityDrawerIn" not in styles and "animation: maturityModalIn" in styles and "@keyframes maturityModalIn" in styles, "project creation must not use right-side drawer motion")
    ...
    require("下一步：选择模板" in component and "下一步：确认信息" in component and "创建并进入评估模板" in component and "创建并进入评分执行" in component, "create modal must expose one primary action per step")
    close_create_block = component.split("function closeCreateWizard()", 1)[1].split("function saveCreateDraft", 1)[0]
    create_keyboard_block = component.split("if (!model.createOpen) return;", 1)[1].split('if (event.key !== "Tab") return;', 1)[0]
    require(
        ...
        and 'if (event.key === "Escape")' in create_keyboard_block
        and "closeCreateWizard();" in create_keyboard_block
    ...
    require(component.count('data-maturity-literal="project-owner"') >= 3, "project owner must remain a literal value in confirmation, expanded preview and project detail views")
    ...
    require(all(token in batch_block for token in ("apply-focus-batch-level", "apply-focus-target-batch-level", "data-maturity-focus-batch-slider", 'type="range" min="1" max="5" step="1"', 'data-maturity-min-level="${minimumIndex + 1}"', 'data-maturity-max-level="${maximumIndex + 1}"', "下级评估设置", "下级目标状态设置", "清空下级所有评分", "统一设置下级当前状态", "统一设置下级目标状态", "focusCurrentMaximumLevel", "focusTargetMinimumLevel", "focusExistingTargetMinimumLevel", "focusCurrentMaximumAllowedLevel", "目标状态不能低于下级当前最高等级", "当前状态不能高于下级已有最低目标等级", "batch.current.hasAny", "batch.target.hasAny && existingTargetMinimumLevel")) and all(token in component for token in ('return focusCurrentMaximumLevel(detail, items);', 'return focusExistingTargetMinimumLevel(detail, items) || "L5";', "dataset.maturityMinLevel", "dataset.maturityMaxLevel", "Math.max(minimumValue, Math.min(maximumValue, requestedValue))", "event.target.value = String(value)", "DIMENSIONS.reduce", "作为下级评估点的四维初始等级")) and all(token not in component for token in ("目标状态必须高于下级当前最高等级", "当前状态必须低于下级已有最低目标等级", "无法统一设置更高目标", "currentMaximumIndex + 1", "targetMinimumIndex - 1")), "focus initialization must retain fixed L1-L5 slider geometry, show existing current maximum and target minimum instead of stale slider memory, and enforce the order-independent inclusive target-greater-than-or-equal-to-current rule")
    require('const currentLevel = batch.current.hasAny\n      ? currentMaximumLevel' in batch_block and 'const targetLevel = batch.target.hasAny && existingTargetMinimumLevel\n      ? existingTargetMinimumLevel' in batch_block, "locked child batch rows must display the actual current maximum and existing target minimum instead of stale requested slider values")
    require(all(token in element_controls_block for token in ("currentLevelIsAboveTarget", "aboveTarget", "当前状态不能高于目标状态", "currentCeilingConflict", "allowedCurrentRange")) and all(token in component for token in ("复核状态不能高于目标状态", "if (currentLevelIsAboveTarget(entry, dimension, level))", "if (currentLevelIsAboveTarget(entry, event.target.dataset.scoreDimension, event.target.value))", "if (currentLevelIsAboveTarget(entry, event.target.dataset.scoreReviewDimension, event.target.value))")), "current and review writes must enforce the same-dimension target ceiling before local persistence regardless of edit order")
    require(all(token in batch_block for token in ("hasAnyScore", "hasAnyTarget", "hasAllScores", "hasAllTargets", "current:", "target:", "hasAll:", "canApply", "canClear", "清空后可重新统一设置")), "child current and target initialization must lock, complete and clear independently")
    require('model.focusBatchOpen = !batch.target.hasAll' in component and 'model.focusBatchOpen = !batch.current.hasAll' in component and component.count("model.focusBatchOpen = false") == 1, "applying the first child current or target batch must keep settings open and only the second completed batch may close them")
    require('.maturity-v9-score-slider-track > input[type="range"]:disabled { cursor: not-allowed; opacity: 1; }' in styles, "disabled batch slider thumbs must remain opaque so the track and tick cannot show through the circular center")
    ...
    require(all(token in component for token in ("request-clear-focus-targets", "confirm-clear-focus-targets", "cancel-clear-focus-targets", 'entry.targetElements = {};', 'entry.targetDimensionNotes = {};', 'scope: "FOCUS_TARGET_CLEAR"')), "clearing child targets must be confirmed and must not remove current scores")
    ...
    require("focus?.description" in scoring_block and "maturity-v5-focus-stats" in scoring_block and "focusApplicableCount" in scoring_block and "focusCompletedApplicableCount" in scoring_block and "currentIndex" not in scoring_block and "targetLevel" not in scoring_block, "focus execution header must show definition, applicability and completed/applicable counts without feeding child aggregate scores back into the scoring header")
    ...
    require(all(token in component for token in ("function normalizeTargetFields", 'hasOwnProperty.call(entry, "targetElements")', "entry.targetLevel", 'hasOwnProperty.call(entry, "targetDimensionNotes")', "entry.targetReason", "normalizeScoreEntries")), "legacy browser-local target level and reason must hydrate into four target dimensions without replacing existing new fields")
    require("target-dimensions-20260720-1" in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"), "the paired current/target workbench assets must use a fresh shared cache version")
    ...
    require(
        ...
        and 'maturity-project-info-profile-20260720-1' in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"),
    ...
    require(component.index('class="maturity-v6-project-sticky-header"') < component.index('class="maturity-v21-project-tab-row"') < component.index('class="maturity-v1-tabs"') and 'aria-label="当前项目与项目步骤"' in component and "maturity-v5-project-context" not in component, "the locked project region must retain only the shared six-step navigation after project identity moves to the App Shell header")
    ...
    require(all(token in element_controls_block for token in ("data-maturity-score-group", "LEVELS.map", "maturity-v15-score-matrix", 'renderLane(key, label, "current")', 'renderLane(key, label, "target")', "data-score-target-dimension-note")) and "maturity-v3-level-buttons" not in element_controls_block and "set-element-level" not in element_controls_block, "every dimension must render paired current and target five-cell score matrices with separate explanations")
    require("commitScoreLevel" in score_update_block and "rubricRows(item, dimension)" in score_update_block and "updateScoreEntry" in score_update_block and all(token in score_update_block for token in ("reviewElements", "elements", "targetElements", "syncLegacyTargetProjection")), "matrix selections must validate the object rubric and update current, review or target fields through the existing score-entry contract")
    require(all(token in element_controls_block for token in ("currentDimensionLevel", "targetLevelIsBelowCurrent", "belowCurrent", "目标状态不能低于当前状态", "maturity-v33-target-floor-message")) and all(token in score_update_block for token in ("targetLevelIsBelowCurrent", "showToast", '"error"')), "target matrices must disable levels below the same-dimension current state and retain a defensive action guard")
    require(all(token in styles for token in ("grid-template-columns: minmax(180px, 0.7fr) minmax(300px, 1.35fr) minmax(310px, 1fr)", ".maturity-v33-focus-batch-row.is-target", "background: #a3681f", ".maturity-v1-button.is-primary.is-target-action")), "both child-setting rows must share fixed grid tracks while target controls use the established warm Apple Shell target semantic")
    ...
    require(all(token in inspector_block for token in ("maturity-v12-score-layout", "maturity-v12-score-dimensions", "maturity-v12-score-side", "maturity-v33-target-validation", "renderScoreOverview")) and "maturity-v16-target-settings" not in inspector_block and 'data-score-text="targetReason"' not in inspector_block and all(token in element_controls_block for token in ("data-score-dimension-note", "data-score-target-dimension-note", "${stateLabel}评分说明（可选）")), "score execution must replace the scalar target block with paired per-dimension current and target explanations")
    ...
    require(all(token in score_overview_block for token in ("maturity-v15-score-overview", "maturity-v17-score-overview-heading", "data-maturity-point-radar", "当前状态", "目标状态", "maturity-v16-score-target", "pointResult?.targetLevel")) and all(token not in score_overview_block for token in ("总体目标（等轴）", "非逐维目标", "<details", "<summary", "项目适用评估点", "目标达成率", "强项", "待加强", "maturity-v15-score-observation", "维度得分明细")), "the fixed-open score overview must use the backend per-dimension target result without an equal-axis placeholder")
    require(all(token in component for token in ('<canvas width="320" height="178" data-maturity-point-radar', "const labelOffset = 18", "y: cssHeight / 2", "cssHeight / 2 - labelOffset - 8", "radius + labelOffset")) and ".maturity-v15-point-radar canvas { max-height: none; }" in styles, "the scoring radar must preserve its drawing aspect ratio and reserve symmetric label-safe space")
    ...
    require(all(token in frontend_spec for token in ("第四十轮逐维当前/目标评分", "targetElements", "清空下级所有目标", "同维度当前状态", "旧 14 列评分表继续可导入")) and all(token in global_plan for token in ("FE-R193", "FE-R194", "FE-R195", "同维度当前状态", "sapd-maturity-v2.2.0")) and all(token in business_spec for token in ("0.1.50 逐维当前/目标评分", "targetDimensionResults", "统一设置下级目标状态", "同维度当前状态")), "all three maturity contracts must synchronize per-dimension targets, same-dimension target floors, legacy migration and independent child-target controls")
    ...
    require('<strong>${escapeHtml(level)}</strong></button>' in element_controls_block and '<span>${escapeHtml(rubric?.levelName' not in element_controls_block and "selectedRubric?.levelName || LEVEL_NAMES[selectedLevel]" in element_controls_block and "${stateLabel}等级定义" in element_controls_block and all(token in styles for token in ("font-size: 17px", ".maturity-v15-score-feedback > span", "font-size: 13px")), "score cells must show only enlarged L1-L5 labels while each current/target lane carries its object rubric definition")
    ...
    require(all(token in component for token in ("drawHierarchyRadar", "data-maturity-hierarchy-radar", "dimensionResults", "targetDimensionResults", "当前状态", "目标状态")) and "总体目标（等轴）" not in component and "非逐维目标" not in component, "L0, L1 and L2 statistics must render same-grain current and per-dimension target radars")
    ...
    require("current_index is not None and target_index is not None" in maturity_py and "current_index is not None and target_index is not None and bool(target_reason)" not in maturity_py and "适用项必须填写有效的 L1—L5 四维目标" in maturity_py, "backend completion and score-import validation must require four target dimensions while keeping all explanations optional")
    require(all(token in component for token in ("renderResultDimensionRadar", "drawResultDimensionRadar", "data-maturity-result-radar", "summary.dimensionResults", "summary.targetDimensionResults", "目标虚线来自四个维度")) and all(token in styles for token in (".maturity-v15-result-profile-grid", ".maturity-v15-result-dimension-radar", ".maturity-v15-result-dimension-layout")), "assessment results must add a backend current/target four-dimension radar next to coverage while preserving the all-L2 capability radar")
    ...
    require(all(token in scoring_block for token in ("maturity-v18-focus-heading", "maturity-v4-focus-applicability", "focusTotalCount", "focusApplicableCount", "focusCompletedApplicableCount", "<dt>完成</dt>", "<dt>适用性</dt>", '${focusCompletedApplicableCount}/${focusApplicableCount}', '${focusApplicableCount}/${focusTotalCount}')) and "focusResolvedCount" not in scoring_block and "关注点适用" not in scoring_block, "focus completion must be completed/applicable while applicability remains applicable/total")
    ...
    require(all(token in review_block for token in ("检查对象", "data-maturity-review-dimension", "当前状态", "目标状态", "返回评分", "renderReviewInspector(detail, item, focus)")) and "renderReviewQueueGroup" in review_tab_block and "renderScoreInspector" not in review_tab_block and "data-score-field" not in review_block and "data-score-text" not in review_block and all(token in styles for token in (".maturity-v17-review-inspector", ".maturity-v17-review-dimensions", ".maturity-v33-review-state-pair")), "score checking must use an independent read-only current/target summary instead of nesting the editable scoring workbench")
    ...
    require(all(token in component for token in ("function flushScoreInspector", "async function saveAndAdvanceScoreItem", "await model.calculationPromise", "await calculateDetail(detail, { silent: true })")), "save-and-next must flush the current inspector and await authoritative backend calculation before navigation")
    ...
    require(all(token in component for token in ("scheduleScoringLanding", "firstMissing", "missingDimension", ".maturity-v12-score-dimensions", ".maturity-v1-project-page", "dimensionOverflow", "preventScroll: true")) and "maturity-v33-target-validation" in component, "blocker adjustment must land on the exact current/target score item in either a local or shared scoring scroll owner")
    ...
    require(all(token in component for token in ("renderOverallCapabilityTop10", 'title: "L2 能力 Top 10"', "成熟度领先 Top 10", "改进优先 Top 10", "maturity-v36-ranking-columns", "当前成熟度", "目标成熟度", "renderDimensionPriorityTop10", "renderDimensionRankingRows", "leadingRows", "improvementRows", "维度评估 L2 能力 Top 10", "priorityByCapability", "dimensionResults?.[key]", "priorityScore", "renderResultEvaluation", "L1 优势能力域", "L1 重点加强", "四维均衡", "证据基础")) and all(token in styles for token in (".maturity-v33-overall-rankings", ".maturity-v34-overall-list", ".maturity-v36-ranking-columns", ".maturity-v34-result-insights", ".maturity-v33-dimension-grid", ".maturity-v33-dimension-rankings")) and "priorityThreshold" not in component and "dimensionPriorityThreshold" not in component, "results must pair source-backed leading and improvement Top 10 rankings overall, expose current and target maturity as independent columns, and preserve source-backed narrative evaluation without inventing frontend thresholds")
    require(all(token in results_tab_block for token in ("L2 能力分数清单", 'title: "L2 能力 Top 10"', "维度评估 L2 能力 Top 10")) and "总体评估 L2 能力 Top 10" not in results_tab_block and all(token not in results_tab_block for token in ("优先级与分数沿用后端结果", "后端维度聚合与总体优先级")), "result section names must use customer-facing score and ranking language without implementation-owner captions")
    require(all(token in component for token in ("maturityDistribution", "subCategoryResults", "gapItems", "dimensionResults", "evidenceDistribution", "特别是", "位列改进优先首位")) and "差距与优先级均沿用后端结果，不在前端重算。" not in component, "result evaluation must explain maturity distribution, L1 strengths and improvement areas, the leading L2 gap, four-dimension balance and evidence coverage without implementation-owner copy")
    require(all(token in internal_details_block for token in ("评估证据", "已填报", "未填报", 'row.status === "scored" ? "已完成"', 'row.status === "not_applicable" ? "不适用"', 'const isService = row.itemType === "SERVICE"', 'const typeLabel = isService ? ""')) and '${escapeHtml(row.itemType)}' not in internal_details_block, "internal score details must hide raw SERVICE labels, localize row status and show whether assessment evidence was submitted")
    ...
    require(all(token in maturity_py for token in ("target_dimension_conflicts", "targetDimensionConflicts", "target_below_current", "targetBelowCurrent", "minimumTargetDimensions", "statisticsReady", "resultAvailability")) and all(token in component for token in ("scoreTargetConflict", "scoringNavigationBlocked", "targetConflict: true", "targetConflictMessage", "不能低于当前状态")), "backend same-dimension target validation and frontend navigation must share one authoritative conflict contract")
    require(all(token in results_tab_block for token in ('!statisticsReadyForDisplay(detail)', "评分完整前不输出成熟度统计、雷达或优先级", "open-review-tab")) and all(token in component for token in ("function statisticsReadyForDisplay", 'summary.statisticsReady === true', '["completed", "reported", "archived"]', "summary.notScoredCount", "detail?.resultStale", "const blockingCount", "const canComplete")) and all(token in complete_assessment_block for token in ("liveSummary.statisticsReady !== true", "finalSummary.statisticsReady !== true")), "only incomplete applicable scores or backend target conflicts may block project completion and result statistics")
    ...
    require(all(token in component for token in ("controlledDemoRevision", "storedProject.controlledDemoRevision", "locallyStored: false")) and all(token in maturity_py for token in ("target_conflict_count=3", "remaining_target_conflicts", 'project_profile: str = "development"', 'normalized_profile == "delivery"', '"demo-project-005"')) and all(token in api_server_source for token in ("maturity_workspace_project_profile", 'RUNTIME_LABEL == "bundle"', "project_profile=maturity_workspace_project_profile()")) and "project_profile=projection_api.maturity_workspace_project_profile()" in bundle_server_source, "development must expose five controlled projects while bundle runtimes select the two-case delivery profile")
    require(all(token in component for token in ("function renderFeedback", "maturity-v24-feedback", "toastRoute", "normalizedRoute", "dismiss-feedback")) and 'insertAdjacentHTML("beforeend"' not in feedback_block and all(token in styles for token in (".maturity-v24-feedback", "grid-template-columns: auto minmax(0, 1fr) auto")), "maturity feedback must stay inside the owning route and must not float over the global top-right shell")
    ...
    require(all(token in project_list_block for token in ("评估项目管理", "renderTemplateManager()")) and all(token in template_manager_block for token in ("模板管理", "全部模板", "自定义模板", "导入任务", "new-template", "trigger-global-template-import", "新增模板", "导入自定义模板", "项目来源模板只能回到原项目修改，也可创建独立副本")) and all(token in component for token in ("export-global-template", "进入项目优化", "创建副本")) and "模板管理将在正式持久化阶段开放" not in component, "the maturity homepage must visibly own project progress and a standalone custom-template create/import/export manager")
    ...
    require(all(token in report_tab_block for token in ("maturity-v37-report-shell", "renderReportOverall(reportDetail)", "renderReportCategoryCoverage(reportDetail)", "renderReportPriorityTable(reportDetail)", "renderReportResultAppendix(reportDetail)", "data-maturity-capability-radar", "data-maturity-result-radar")) and all(token in component for token in ("评估概况", "关键发现", "提升建议", "下一步计划", "data-maturity-report-field", "能力类别评分", "Boolean(text(report.html).trim())", "Boolean(text(report.markdown).trim())")) and "report-limitations" not in component and "口径与限制" not in report_tab_block and component.count('data-maturity-action="download-report"') == 2 and all(token in component for token in ('data-format="markdown"', 'data-format="html"', 'if (format === "markdown") downloadBlob', 'if (format === "html") downloadBlob')) and all(token in styles for token in (".maturity-v37-report-shell", ".maturity-v37-report-nav", ".maturity-v37-report-dashboard", ".maturity-v37-report-narrative", ".maturity-v37-report-appendix", ".maturity-v37-report-appendix .maturity-v23-result-section-meta", ".maturity-v37-report-appendix .maturity-v2-internal-details summary small { display: none; }", "project identity is owned by the shared page header", "top: 52px", "top: 60px")) and ".maturity-v1-project-page:has(.maturity-v37-report-shell) .maturity-v21-project-tab-row" not in styles and 'data-format="json"' not in component and 'data-format="package"' not in component and all(token in report_config for token in ("markdown:", "html:", "sapd-maturity-report-model-v2", "executiveSummary", "keyFindings", "managementRecommendations", "nextSteps", "radar_suite", "score_appendix")) and "json:" not in report_config, "assessment report UI and configuration must expose the selected executive workspace, editable narratives, real result radars, full appendices without the removed limitations card, stable tabs and two independent Markdown/HTML downloads")
    require(all(token in component for token in ("function formalAssessmentReady(detail)", "function reportMatchesCurrentAssessment(detail, report)", "async function restorePersistedReports()", "refreshHydratedAssessments", "getMaturityReportArtifact", "reportRun.inputHash", "reportVersion.resultHash", 'operation: updateMode ? "update" : "create"', 'updateMode ? "更新评估报告" : "生成评估报告"')) and "detail.result = clone(report" not in component and all(token in api_server_source for token in ("maturity_report_storage_root", "persist_maturity_report_artifact", "load_maturity_report_artifact", "create_and_persist_maturity_report", 'report.get("ok") is not True')) and "if (!detail.report) generateReport(detail);" not in component, "refresh must recalculate current scores before restoring only a hash-matched formal report and must never replace the live result with report data")
    require(all(token in data_client for token in ("inputHash", "resultHash", 'params.set("input_hash"', 'params.set("result_hash"')) and all(token in api_server_source for token in ("input_hash", "result_hash", "matches_result_version", "calculation_run.get(\"inputHash\")", "report_version.get(\"resultHash\")")), "report restoration must search local history by the live input and result hashes instead of assuming the latest artifact matches")
    ...
    require('detail?.project?.templateType === "base" || detail?.template?.type === "base"' in display_template_name_block and 'return "SAPD标准模板"' in display_template_name_block and "detail.project" not in display_template_name_block.split("return", 1)[0], "base projects must use the canonical current default-template display name without mutating project records")
    ...
    require('const currentLevelName = LEVEL_NAMES[currentLevel] || ""' in result_summary_block and 'Number(canvas.dataset.radarHeight || 390)' in component and 'Number(canvas.dataset.radarRadius || 150)' in component and 'cssWidth * 0.34' in component and 'cssHeight / 2 - labelOffset - 8' in component and all(token in styles for token in ("grid-template-rows: auto minmax(440px, 1fr)", ".maturity-v25-compact-dimension-radar aside dl", "grid-template-columns: repeat(2, minmax(0, 1fr))")), "result title and the radar visual column must use the named level, an enlarged label-safe four-dimension radar, report-specific geometry and compact value cards")
    require(all(token in frontend_spec for token in ("第二十六轮评分滚动、摘要基线与双栏首页裁定", "评分表单是唯一纵向滚动所有者", "SAPD标准模板")) and all(token in global_plan for token in ("FE-R150", "FE-R151", "FE-R152", "FE-R153", "FE-R154", "FE-R155")) and all(token in business_spec for token in ("0.1.39", "完整评分表单是唯一纵向滚动区域", "项目情况 `2/3`")), "all three maturity documents must synchronize round 26 scroll ownership, summary alignment, radar scale, named result and workspace proportions")
    ...
    require(all(token in styles for token in ("thirty-first acceptance: narrow point status and align each L2 readout group", "--maturity-l2-readout-size: 28px", "minmax(210px, 0.65fr)", ".maturity-v22-maturity-summary > div {\n  justify-items: stretch;\n  text-align: left;", ".maturity-v22-maturity-summary dt {\n  width: 100%;\n  justify-content: flex-start;\n  text-align: left;", ".maturity-v22-maturity-summary dd {\n  width: 100%;\n  justify-content: center;\n  text-align: center;", ".maturity-v22-point-summary > div {\n  width: 100%;\n  display: grid;", "align-self: center")) and "maturity-project-info-profile-20260720-1" in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"), "current and target maturity titles must align left while their readouts stay centered, share the 28px maturity/dimension token and allocate less width to point status")
    require("scoring overview: pin below the project header without the static L2 summary offset" in styles and ".maturity-v12-score-side {\n    top: 108px;\n  }" in styles and "maturity-project-info-profile-20260720-1" in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"), "the scoring overview must stay pinned below the project header without adding the static L2 summary height")
    require(all(token in overview_block for token in ("总体完成", "总计 ${progress.capabilityTotalCount} · 适用 ${progress.capabilityCount}", "总计 ${progress.focusTotalCount} · 适用 ${progress.focusCount}", "总计 ${progress.itemTotalCount} · 适用 ${progress.applicableItemCount}", "修改评估分数", "visibleProjectHistory", "每页 3 条", "renderUnlockConfirmation")) and "maturity-v10-project-identity" not in overview_block and all(token in component for token in ("LOCKED_ASSESSMENT_STATUSES", "unlockAssessmentForEditing", 'detail.project.status = "score_review"', "ASSESSMENT_UNLOCKED", "ASSESSMENT_COMPLETED", 'detail.project.changeHistory = list(detail.project.changeHistory);', 'action === "step-project-history"')) and any(token in component for token in ('detail.project.readOnly = true', 'persistedDetail.project.readOnly = true')) and all(token in project_search_block for token in ("page-search-control", "page-search-input-shell", "page-search-match-status", "step-project-search")) and all(token in styles for token in ("compact equal-height cards, paged history and capability-mapping search grammar", ".maturity-v32-project-history > footer", "width: var(--page-local-search-width", ".maturity-v32-unlock-modal", ".maturity-v21-project-search.page-search-control")) and '"readOnly": status in {"completed", "reported", "archived"}' in maturity_py and "maturity-project-info-profile-20260720-1" in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"), "completed assessments must expose compact equal-height cards, unbounded paged history, confirmed score unlocking and the capability-mapping local-search grammar")
    require(all(token in overview_block for token in ("编辑项目信息", "renderProjectInfoEditor(detail)")) and all(token in component for token in ("function openProjectInfoEditor", "function saveProjectInfo", "PROJECT_INFO_UPDATED", "customerContextSnapshot", "invalidateResult: true", "invalidateReport: true", "scheduleCalculation(detail)", "计划结束时间不能早于开始时间")) and '["name", "organization", "owner", "assessors", "customerCharacteristics", "constraints"].forEach' not in component and all(token in styles for token in (".maturity-v37-project-heading-actions", ".maturity-v37-project-edit-workspace")), "project overview must edit and persist project metadata without seed hydration rolling user fields back")
    require(all(token in overview_block for token in ("renderReportDownloadConfirmation", "reportDownloadAvailable", "assessmentLocked || reportPreviouslyGenerated(detail)", "index === 5 && reportDownloadAvailable", "下载评估报告")) and all(token in component for token in ("confirmOverviewReportDownload", "request-overview-report-download", "confirm-overview-report-download", "maturity-report-download-format", "确认并下载", "reportCapabilityRadarGroups", "reportRadarData", "TAB_STORAGE_KEY", "rememberedProjectTab", "rememberProjectTab", "nextIndex = (index + 1) % normalizedValues.length")) and all(token in styles for token in (".maturity-v33-report-download-entry", ".maturity-v33-report-download-modal", "grid-template-rows: 26px minmax(15px, auto) 28px")) and ".maturity-v1-project-page:has(.maturity-v37-report-shell) > .maturity-v1-project-body" not in styles and "maturity-project-info-profile-20260720-1" in (ROOT / "frontend" / "capability-browser" / "index.html").read_text(encoding="utf-8"), "completed assessments must place report download under step six, restore the active project tab, share page-body geometry and render report-page radars with the same adjacent-axis rule as HTML")
    ...
    require(all(token in frontend_spec for token in ("第三十三轮结果排行与评分明细术语裁定", "左右并列", "成熟度领先 Top 10", "改进优先 Top 10", "需要加强的 L1 能力域", "已填报 / 未填报")) and all(token in global_plan for token in ("FE-R169", "FE-R170", "FE-R171", "FE-R183", "前端不得引入新的优先级阈值")), "the frontend contracts must synchronize source-backed leading/improvement rankings, narrative evaluation and Chinese internal-detail terminology")
    require(all(token in component for token in ("ROADMAP_STATUSES", "improvementRoadmapRows", "renderImprovementRoadmap", "data-maturity-roadmap-field", "improvementRoadmap: improvementRoadmapRows(detail)", "内容待更新")) and "dueDate" not in component.split("function improvementRoadmapRows", 1)[1].split("function templateLibraryRecords", 1)[0] and all(token in styles for token in ("official gap-to-action improvement roadmap", ".maturity-v35-roadmap-scroll", "overflow-x: auto", ".maturity-v35-roadmap-table")) and all(token in maturity_py for token in ("IMPROVEMENT_ROADMAP_STATUSES", '"improvementRoadmap": improvement_roadmap', "roadmap_rows_html", "## 改进路线图")) and "完成期限" not in maturity_py.split("def create_maturity_report_snapshot", 1)[1] and "improvement_roadmap" in report_config, "the result page must persist an official-gap-based improvement roadmap without a completion-date field and synchronize it into the report snapshot")
    require(all(token in frontend_spec for token in ("第三十六轮改进路线图与报告同步裁定", "按 `capabilityId`", "Markdown、自包含 HTML")) and all(token in global_plan for token in ("FE-R184", "改进路线图", "纳入快照哈希")), "the frontend contracts must synchronize the source-owned improvement roadmap and dual-format report parity")
    ...
    require(all(token in frontend_spec for token in ("第三十九轮项目信息编辑与开发/交付测试数据分层裁定", "development", "delivery")) and all(token in global_plan for token in ("FE-R191", "FE-R192", "development=5", "delivery=2")) and all(token in business_spec for token in ("0.1.49", "项目信息维护", "测试数据环境分层")), "all three maturity documents must synchronize project metadata editing and the five-development/two-delivery test-data contract")
