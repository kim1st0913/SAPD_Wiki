...
def _isolated_root(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            "runtime root must be a protected absolute MCP directory"
        ) from exc
...
def _read_secret(path: Path) -> bytes:
    try:
        ...
        raise ValueError("runtime secret is unavailable") from exc
    if len(value) < 32:
...
def run_dev_sidecar(
    *,
    ...
    try:
        delivered_secret = receive_one_shot_secret(
            secret_channel_endpoint,
            channel_kind=secret_channel_kind,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        "--secret-channel-fd",
        dest="secret_channel_endpoint",
        required=True,
        type=int,
    )
    parser.add_argument(
        "--secret-channel-kind",
        choices=("posix-fd", "windows-handle"),
    )
    ...
    run_dev_sidecar(
        ...
        secret_channel_endpoint=args.secret_channel_endpoint,
